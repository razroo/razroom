...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/razroom/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/razroom");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/razroom/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__RAZROOM_CONTROL_UI_BASE_PATH__ = "/razroom";
    const app = mountApp("/razroom/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/razroom");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/razroom/sessions");
  });
