...
This document describes how the Razroom ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`razroom acp` exposes an ACP agent over stdio and forwards prompts to a running
Razroom Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `razroom acp` and speaks ACP messages over stdio.
...
`razroom acp` resolves the Gateway URL and auth from CLI flags or config:
