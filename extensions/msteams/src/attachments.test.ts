...
const detectMimeMock = mock(async () => "image/png");
const saveMediaBufferMock = mock(async () => ({
  path: "/tmp/saved.png",
  ...
  describe("downloadMSTeamsAttachments", () => {
    ...
    it("retries with auth when the first request is unauthorized", async () => {
      ...
      const media = await downloadMSTeamsAttachments({
        attachments: [{ contentType: "image/png", contentUrl: "https://x/img" }],
        maxBytes: 1024 * 1024,
        tokenProvider: { getAccessToken: mock(async () => "token") },
        allowHosts: ["x"],
        authAllowHosts: ["x"],
        fetchFn: fetchMock as unknown as typeof fetch,
      });
    ...
    it("skips auth retries when the host is not in auth allowlist", async () => {
      const { downloadMSTeamsAttachments } = await load();
      const tokenProvider = { getAccessToken: mock(async () => "token") };
      const fetchMock = mock(async (_url: string, opts?: RequestInit) => {
        const hasAuth = Boolean(
          opts &&
          typeof opts === "object" &&
