...
mock("music-metadata", () => ({
  ...
  parseBuffer: mock().mockResolvedValue({ format: {} }),
}));

mock("@vector-im/matrix-bot-sdk", () => ({
  ConsoleLogger: class {
    trace = mock();
    debug = mock();
    info = mock();
  ...
  MatrixClient: mock(),
  SimpleFsStorageProvider: mock(),
  RustSdkCryptoStorageProvider: mock(),
}));

const loadWebMediaMock = mock().mockResolvedValue({
  buffer: Buffer.from("media"),
  fileName: "photo.png",
  contentType: "image/png",
  kind: "image",
});
const mediaKindFromMimeMock = mock(() => "image");
const isVoiceCompatibleAudioMock = mock(() => false);
const getImageMetadataMock = mock().mockResolvedValue(null);
const resizeToJpegMock = mock();
...
const makeClient = () => {
  const sendMessage = mock().mockResolvedValue("evt1");
  const uploadContent = mock().mockResolvedValue("mxc://example/file");
  const client = {
    sendMessage,
    uploadContent,
    getUserId: mock().mockResolvedValue("@bot:example.org"),
...
describe("sendMessageMatrix media", () => {
  ...
  it("uploads encrypted media with file payloads", async () => {
    ...
    (client as { crypto?: object }).crypto = {
      isRoomEncrypted: mock().mockResolvedValue(true),
      encryptMedia: mock().mockResolvedValue({
