...
describe("resolveMatrixRoomId", () => {
  it("uses m.direct when available", async () => {
    ...
    const client = {
      getAccountData: mock().mockResolvedValue({
  ...
  it("falls back to joined rooms and persists m.direct", async () => {
    ...
    const setAccountData = mock().mockResolvedValue(undefined);
    const client = {
      getAccountData: mock().mockRejectedValue(new Error("nope")),
      getJoinedRooms: mock().mockResolvedValue([roomId]),
      getJoinedRoomMembers: mock().mockResolvedValue(["@bot:example.org", userId]),
      setAccountData,
    } as unknown as MatrixClient;
  ...
  it("continues when a room member lookup fails", async () => {
    ...
    const client = {
      getAccountData: mock().mockRejectedValue(new Error("nope")),
      getJoinedRooms: mock().mockResolvedValue(["!bad:example.org", roomId]),
  ...
  it("allows larger rooms when no 1:1 match exists", async () => {
    ...
    const client = {
      ...
      setAccountData: mock().mockResolvedValue(undefined),
    } as unknown as MatrixClient;
