...
describe("downloadMatrixMedia", () => {
  const saveMediaBuffer = mock().mockResolvedValue({
    path: "/tmp/media",
  ...
  it("decrypts encrypted media when file payloads are present", async () => {
    const decryptMedia = mock().mockResolvedValue(Buffer.from("decrypted"));

    const client = {
      crypto: { decryptMedia },
      mxcToHttp: mock().mockReturnValue("https://example/mxc"),
