...
mock("./channel.js", () => ({
  publishNostrProfile: mock(),
  getNostrProfileState: mock(),
}));

// Mock the import module
mock("./nostr-profile-import.js", () => ({
  importProfileFromRelays: mock(),
  mergeProfiles: mock((local, imported) => ({ ...imported, ...local })),
}));
...
function createMockContext(overrides?: Partial<NostrProfileHttpContext>): NostrProfileHttpContext {
  return {
    getConfigProfile: mock().mockReturnValue(undefined),
    updateConfigProfile: mock().mockResolvedValue(undefined),
    getAccountInfo: mock().mockReturnValue({
...
describe("nostr-profile-http", () => {
  ...
  describe("GET /api/channels/nostr/:accountId/profile", () => {
    it("returns profile and publish state", async () => {
      const ctx = createMockContext({
        getConfigProfile: mock().mockReturnValue({
          name: "testuser",
  ...
  describe("POST /api/channels/nostr/:accountId/profile/import", () => {
    ...
    it("auto-merges when requested", async () => {
      const ctx = createMockContext({
        getConfigProfile: mock().mockReturnValue({ about: "local bio" }),
      });
      const handler = createNostrProfileHttpHandler(ctx);
      const req = createMockRequest("POST", "/api/channels/nostr/default/profile/import", {
        autoMerge: true,
    ...
    it("returns error when account not found", async () => {
      const ctx = createMockContext({
        getAccountInfo: mock().mockReturnValue(null),
      });
      const handler = createNostrProfileHttpHandler(ctx);
