...
const sendBlueBubblesAttachmentMock = vi.hoisted(() => mock());
const sendMessageBlueBubblesMock = vi.hoisted(() => mock());
const resolveBlueBubblesMessageIdMock = vi.hoisted(() => mock((id: string) => id));
...
type RuntimeMocks = {
  detectMime: ReturnType<typeof mock>;
  fetchRemoteMedia: ReturnType<typeof mock>;
};
...
function createMockRuntime(): { runtime: PluginRuntime; mocks: RuntimeMocks } {
  const detectMime = mock().mockResolvedValue("text/plain");
  const fetchRemoteMedia = mock().mockResolvedValue({
