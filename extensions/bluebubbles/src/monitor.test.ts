...
// Mock dependencies
mock("./send.js", () => ({
  resolveChatGuidForTarget: mock().mockResolvedValue("iMessage;-;+15551234567"),
  sendMessageBlueBubbles: mock().mockResolvedValue({ messageId: "msg-123" }),
}));

mock("./chat.js", () => ({
  markBlueBubblesChatRead: mock().mockResolvedValue(undefined),
  sendBlueBubblesTyping: mock().mockResolvedValue(undefined),
}));

mock("./attachments.js", () => ({
  downloadBlueBubblesAttachment: mock().mockResolvedValue({
    buffer: Buffer.from("test"),
    contentType: "image/jpeg",
  }),
}));

mock("./reactions.js", async () => {
  const actual = await vi.importActual<typeof import("./reactions.js")>("./reactions.js");
  return {
    ...actual,
    sendBlueBubblesReaction: mock().mockResolvedValue(undefined),
  };
});

// Mock runtime
const mockEnqueueSystemEvent = mock();
const mockBuildPairingReply = mock(() => "Pairing code: TESTCODE");
const mockReadAllowFromStore = mock().mockResolvedValue([]);
const mockUpsertPairingRequest = mock().mockResolvedValue({ code: "TESTCODE", created: true });
const mockResolveAgentRoute = mock(() => ({
  agentId: "main",
  accountId: "default",
  sessionKey: "agent:main:bluebubbles:dm:+15551234567",
}));
const mockBuildMentionRegexes = mock(() => [/\bbert\b/i]);
const mockMatchesMentionPatterns = mock((text: string, regexes: RegExp[]) =>
  regexes.some((r) => r.test(text)),
);
const mockResolveRequireMention = mock(() => false);
const mockResolveGroupPolicy = mock(() => "open");
const mockDispatchReplyWithBufferedBlockDispatcher = mock(async () => undefined);
const mockHasControlCommand = mock(() => false);
const mockResolveCommandAuthorizedFromAuthorizers = mock(() => false);
const mockSaveMediaBuffer = mock().mockResolvedValue({
  path: "/tmp/test-media.jpg",
  contentType: "image/jpeg",
});
const mockResolveStorePath = mock(() => "/tmp/sessions.json");
const mockReadSessionUpdatedAt = mock(() => undefined);
const mockResolveEnvelopeFormatOptions = mock(() => ({
  template: "channel+name+time",
}));
const mockFormatAgentEnvelope = mock((opts: { body: string }) => opts.body);
const mockFormatInboundEnvelope = mock((opts: { body: string }) => opts.body);
const mockChunkMarkdownText = mock((text: string) => [text]);

function createMockRuntime(): PluginRuntime {
  return {
    version: "1.0.0",
    config: {
      loadConfig: mock(() => ({})) as unknown as PluginRuntime["config"]["loadConfig"],
      writeConfigFile: mock() as unknown as PluginRuntime["config"]["writeConfigFile"],
    },
    system: {
      enqueueSystemEvent:
        mockEnqueueSystemEvent as unknown as PluginRuntime["system"]["enqueueSystemEvent"],
      runCommandWithTimeout: mock() as unknown as PluginRuntime["system"]["runCommandWithTimeout"],
    },
    media: {
    ...
    channel: {
      text: {
        ...
        chunkText: mock() as unknown as PluginRuntime["channel"]["text"]["chunkText"],
      ...
      reply: {
        ...
        createReplyDispatcherWithTyping:
          mock() as unknown as PluginRuntime["channel"]["reply"]["createReplyDispatcherWithTyping"],
        resolveEffectiveMessagesConfig:
          mock() as unknown as PluginRuntime["channel"]["reply"]["resolveEffectiveMessagesConfig"],
        resolveHumanDelayConfig:
          mock() as unknown as PluginRuntime["channel"]["reply"]["resolveHumanDelayConfig"],
        dispatchReplyFromConfig:
          mock() as unknown as PluginRuntime["channel"]["reply"]["dispatchReplyFromConfig"],
        finalizeInboundContext: mock(
      ...
      media: {
        fetchRemoteMedia:
          mock() as unknown as PluginRuntime["channel"]["media"]["fetchRemoteMedia"],
        saveMediaBuffer:
      ...
      session: {
        ...
        recordInboundSession:
          mock() as unknown as PluginRuntime["channel"]["session"]["recordInboundSession"],
        recordSessionMetaFromInbound:
          mock() as unknown as PluginRuntime["channel"]["session"]["recordSessionMetaFromInbound"],
        updateLastRoute:
          mock() as unknown as PluginRuntime["channel"]["session"]["updateLastRoute"],
      },
      mentions: {
        buildMentionRegexes:
      ...
      commands: {
        ...
        isControlCommandMessage:
          mock() as unknown as PluginRuntime["channel"]["commands"]["isControlCommandMessage"],
        shouldComputeCommandAuthorized:
          mock() as unknown as PluginRuntime["channel"]["commands"]["shouldComputeCommandAuthorized"],
        shouldHandleTextCommands:
          mock() as unknown as PluginRuntime["channel"]["commands"]["shouldHandleTextCommands"],
      },
...
describe("BlueBubbles webhook monitor", () => {
  ...
  describe("webhook parsing + auth handling", () => {
    ...
    it("does not route to passwordless targets when a password-authenticated target matches", async () => {
      ...
      const sinkStrict = mock();
      const sinkFallback = mock();
  ...
  describe("inbound debouncing", () => {
    it("coalesces text-only then attachment webhook events by messageId", async () => {
      ...
      try {
        ...
        core.channel.debounce.createInboundDebouncer = mock((params: any) => {
          // oxlint-disable-next-line typescript/no-explicit-any
          type Item = any;
          ...
          return {
            ...
            flushKey: mock(async (key: string) => {
              await flush(key);
            }),
          };
        }) as unknown as PluginRuntime["channel"]["debounce"]["createInboundDebouncer"];
