...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: RAZROOM_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: RazroomConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var RAZROOM_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.RAZROOM_TWITCH_ACCESS_TOKEN?.trim();
