...
const mockJoin = mock().mockResolvedValue(undefined);
const mockSay = mock().mockResolvedValue({ messageId: "test-msg-123" });
const mockQuit = mock();
...
// oxlint-disable-next-line typescript/no-explicit-any
const mockOnMessage = mock((handler: any) => {
  messageHandlers.push(handler);
  return { unbind: mockUnbind };
});

const mockAddUserForToken = mock().mockResolvedValue("123456");
const mockOnRefresh = mock();
const mockOnRefreshFailure = mock();
