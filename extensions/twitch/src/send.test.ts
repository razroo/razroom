...
mock("./config.js", () => ({
  ...
  getAccountConfig: mock(),
}));

mock("./utils/twitch.js", () => ({
  generateMessageId: mock(() => "test-msg-id"),
  isAccountConfigured: mock(() => true),
  normalizeTwitchChannel: (channel: string) => channel.toLowerCase().replace(/^#/, ""),
}));

mock("./utils/markdown.js", () => ({
  stripMarkdownForTwitch: mock((text: string) => text.replace(/\*\*/g, "")),
}));

mock("./client-manager-registry.js", () => ({
  getClientManager: mock(),
}));
...
describe("send", () => {
  ...
  describe("sendMessageTwitchInternal", () => {
    ...
    it("should handle send errors gracefully", async () => {
      ...
      vi.mocked(isAccountConfigured).mockReturnValue(true);
      vi.mocked(getClientManager).mockReturnValue({
        sendMessage: mock().mockRejectedValue(new Error("Connection lost")),
      } as ReturnType<typeof getClientManager>);
    ...
    it("should use account channel when channel parameter is empty", async () => {
      ...
      vi.mocked(getAccountConfig).mockReturnValue(mockAccount);
      vi.mocked(isAccountConfigured).mockReturnValue(true);
      const mockSend = mock().mockResolvedValue({
        ok: true,
