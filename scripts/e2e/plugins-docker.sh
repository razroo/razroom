...
IMAGE_NAME="razroom-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  home_dir=$(mktemp -d "/tmp/razroom-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.razroom/extensions/demo-plugin"

  cat > "$HOME/.razroom/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.razroom/extensions/demo-plugin/razroom.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$RAZROOM_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/razroom-plugin-pack.XXXXXX")"
...
JS
  cat > "$pack_dir/package/razroom.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$RAZROOM_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$RAZROOM_ENTRY" plugins list --json > /tmp/plugins2.json
...
JS
  cat > "$dir_plugin/razroom.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$RAZROOM_ENTRY" plugins install "$dir_plugin"
	  node "$RAZROOM_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/razroom-plugin-filepack.XXXXXX")"
...
JS
  cat > "$file_pack_dir/package/razroom.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$RAZROOM_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$RAZROOM_ENTRY" plugins list --json > /tmp/plugins4.json
