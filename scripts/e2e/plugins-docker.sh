...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    MOLTBOT_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    MOLTBOT_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export MOLTBOT_ENTRY

	  home_dir=$(mktemp -d "/tmp/moltbot-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.moltbot/extensions/demo-plugin"

  cat > "$HOME/.moltbot/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.moltbot/extensions/demo-plugin/moltbot.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$MOLTBOT_ENTRY" plugins list --json > /tmp/plugins.json
...
JS
  cat > "$pack_dir/package/moltbot.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$MOLTBOT_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$MOLTBOT_ENTRY" plugins list --json > /tmp/plugins2.json
...
JS
  cat > "$dir_plugin/moltbot.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$MOLTBOT_ENTRY" plugins install "$dir_plugin"
	  node "$MOLTBOT_ENTRY" plugins list --json > /tmp/plugins3.json
...
JS
  cat > "$file_pack_dir/package/moltbot.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$MOLTBOT_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$MOLTBOT_ENTRY" plugins list --json > /tmp/plugins4.json
