...
IMAGE_NAME="razroom-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="razroom-net-e2e-$$"
GW_NAME="razroom-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "RAZROOM_GATEWAY_TOKEN=$TOKEN" \
	  -e "RAZROOM_SKIP_CHANNELS=1" \
	  -e "RAZROOM_SKIP_GMAIL_WATCHER=1" \
	  -e "RAZROOM_SKIP_CRON=1" \
	  -e "RAZROOM_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
