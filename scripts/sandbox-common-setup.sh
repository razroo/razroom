...
BASE_IMAGE="${BASE_IMAGE:-razroom-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-razroom-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=razroom.sandbox=1)
NOTE
