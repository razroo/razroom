...
MODELS_MODE="${MOLTBOT_E2E_MODELS:-${MOLTBOT_E2E_MODELS:-both}}" # both|openai|anthropic
INSTALL_TAG="${MOLTBOT_INSTALL_TAG:-${MOLTBOT_INSTALL_TAG:-latest}}"
E2E_PREVIOUS_VERSION="${MOLTBOT_INSTALL_E2E_PREVIOUS:-${MOLTBOT_INSTALL_E2E_PREVIOUS:-}}"
SKIP_PREVIOUS="${MOLTBOT_INSTALL_E2E_SKIP_PREVIOUS:-${MOLTBOT_INSTALL_E2E_SKIP_PREVIOUS:-0}}"
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  MOLTBOT_BETA=1 MOLTBOT_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  MOLTBOT_VERSION="$INSTALL_TAG" MOLTBOT_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
