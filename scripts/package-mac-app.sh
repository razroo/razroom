...
# Build and bundle Razroom into a minimal .app we can open.
# Outputs to dist/Razroom.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Razroom.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Razroom"
BUNDLE_ID="${BUNDLE_ID:-ai.razroom.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/razroom/razroom/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Razroom/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :RazroomBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :RazroomGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Razroom"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Razroom"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Razroom"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Razroom" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Razroom/Resources/Razroom.icns" "$APP_ROOT/Contents/Resources/Razroom.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Razroom/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying RazroomKit resources"
RAZROOMKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/RazroomKit_RazroomKit.bundle"
if [ -d "$RAZROOMKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/RazroomKit_RazroomKit.bundle"
  cp -R "$RAZROOMKIT_BUNDLE" "$APP_ROOT/Contents/Resources/RazroomKit_RazroomKit.bundle"
else
  echo "WARN: RazroomKit resource bundle not found at $RAZROOMKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Razroom"
killall -q Razroom 2>/dev/null || true
