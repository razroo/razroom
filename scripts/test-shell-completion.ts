 ...
 * `razroom update`. Use it to verify the completion installation flow
...
\`razroom update\`, \`razroom doctor\`, and \`razroom onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Razroom Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`razroom completion --install\` later to enable.`));
    return;
  }
