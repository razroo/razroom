...
SERVER="${MOLTBOT_SERVER:-${CLAWDBOT_SERVER:-l36}}"
RESULT=$(ssh "$SERVER" '/home/admin/moltbot/scripts/sync-claude-code-auth.sh' 2>&1)
EXIT_CODE=$?
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    ssh "$SERVER" 'systemctl --user restart moltbot' 2>/dev/null
else
