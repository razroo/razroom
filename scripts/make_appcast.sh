...
ZIP=${1:?"Usage: $0 Razroom-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/razroom/razroom/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Razroom-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/razroom/razroom/releases/download/v${VERSION}/"}
