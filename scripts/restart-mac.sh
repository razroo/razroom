#!/usr/bin/env bash
# Reset Razroom like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${RAZROOM_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Razroom.app/Contents/MacOS/Razroom"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Razroom"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Razroom"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Razroom"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.razroom.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/razroom-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${RAZROOM_RESTART_LOG:-/tmp/razroom-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${RAZROOM_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.razroom/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  RAZROOM_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node razroom.mjs daemon install --force --runtime node"
      log "  node razroom.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.razroom/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.razroom.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Razroom instances"
kill_all_razroom
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Razroom"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Razroom.app" ]]; then
    APP_BUNDLE="/Applications/Razroom.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Razroom.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Razroom.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Razroom.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set RAZROOM_APP_BUNDLE to your installed Razroom.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node razroom.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node razroom.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".razroom", "razroom.json"), "utf8");
