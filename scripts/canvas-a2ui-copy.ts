...
export function getA2uiPaths(env = process.env) {
  const srcDir = env.MOLTBOT_A2UI_SRC_DIR ?? path.join(repoRoot, "src", "canvas-host", "a2ui");
  const outDir = env.MOLTBOT_A2UI_OUT_DIR ?? path.join(repoRoot, "dist", "canvas-host", "a2ui");
  return { srcDir, outDir };
}

export async function copyA2uiAssets({ srcDir, outDir }: { srcDir: string; outDir: string }) {
  const skipMissing = process.env.MOLTBOT_A2UI_SKIP_MISSING === "1";
  try {
