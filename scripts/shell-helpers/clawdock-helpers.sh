#!/usr/bin/env bash
# ClawDock - Docker helpers for Razroom
# Inspired by Simon Willison's "Running Razroom in Docker"
# https://til.simonwillison.net/llms/razroom-docker
#
# Installation:
...
# Common paths to check for Razroom
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/razroom"
  "${HOME}/workspace/razroom"
  "${HOME}/projects/razroom"
  "${HOME}/dev/razroom"
  "${HOME}/code/razroom"
  "${HOME}/src/razroom"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Razroom at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/razroom"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Razroom not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/razroom/razroom.git ~/razroom"
    echo "  cd ~/razroom && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^RAZROOM_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d razroom-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart razroom-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f razroom-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec razroom-gateway \
    bash -c 'echo "alias razroom=\"./razroom.mjs\"" > /tmp/.bashrc_razroom && bash --rcfile /tmp/.bashrc_razroom'
}
...
clawdock-cli() {
  _clawdock_compose run --rm razroom-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build razroom-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "RAZROOM_GATEWAY_TOKEN=$token" razroom-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" razroom-gateway \
    bash -c './razroom.mjs config set gateway.remote.token "$TOKEN" && ./razroom.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec razroom-gateway \
    bash -c "./razroom.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart razroom-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm razroom-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec razroom-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'razroom config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Razroom${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (razroom alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to razroom project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.razroom)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'razroom channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'razroom status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.razroom.ai${_CLR_RESET}"
  echo ""
}
