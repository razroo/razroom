#!/usr/bin/env bash
# ClawDock - Docker helpers for MoltBot
# Inspired by Simon Willison's "Running MoltBot in Docker"
# https://til.simonwillison.net/llms/moltbot-docker
#
# Installation:
...
# Common paths to check for MoltBot
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/moltbot"
  "${HOME}/workspace/moltbot"
  "${HOME}/projects/moltbot"
  "${HOME}/dev/moltbot"
  "${HOME}/code/moltbot"
  "${HOME}/src/moltbot"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found MoltBot at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/moltbot"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ MoltBot not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/moltbot/moltbot.git ~/moltbot"
    echo "  cd ~/moltbot && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MOLTBOT_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d moltbot-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart moltbot-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f moltbot-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec moltbot-gateway \
    bash -c 'echo "alias moltbot=\"./moltbot.mjs\"" > /tmp/.bashrc_moltbot && bash --rcfile /tmp/.bashrc_moltbot'
}
...
clawdock-cli() {
  _clawdock_compose run --rm moltbot-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build moltbot-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MOLTBOT_GATEWAY_TOKEN=$token" moltbot-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" moltbot-gateway \
    bash -c './moltbot.mjs config set gateway.remote.token "$TOKEN" && ./moltbot.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec moltbot-gateway \
    bash -c "./moltbot.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart moltbot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm moltbot-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec moltbot-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'moltbot config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for MoltBot${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (moltbot alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to moltbot project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.moltbot)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'moltbot channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'moltbot status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.moltbot.ai${_CLR_RESET}"
  echo ""
}
