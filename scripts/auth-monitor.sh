...
#
# Suggested cron: */30 * * * * /home/admin/razroom/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Razroom notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., razroom-alerts)
...
STATE_FILE="$HOME/.razroom/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Razroom if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use razroom
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Razroom to $NOTIFY_PHONE..."
            razroom send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Razroom is down. Re-authenticate with Claude Code CLI." "urgent"
    exit 1
