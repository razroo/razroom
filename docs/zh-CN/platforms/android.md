...
- 你可以在 Gateway 网关机器上运行 CLI（`moltbot`）（或通过 SSH）。
...
- 在 Gateway 网关主机的 `~/.moltbot/moltbot.json` 中设置 `gateway.bind: "tailnet"`。
...
1. 在 Gateway 网关主机上设置 DNS-SD 区域（示例 `moltbot.internal.`）并发布 `_moltbot-gw._tcp` 记录。
...
1. 在 Gateway 网关主机上创建 `~/.moltbot/workspace/canvas/index.html`。
...
Tailnet（可选）：如果两个设备都在 Tailscale 上，使用 MagicDNS 名称或 tailnet IP 而不是 `.local`，例如 `http://<gateway-magicdns>:18793/__moltbot__/canvas/`。
