...
**推荐：** 运行 `moltbot configure --section web`。它将密钥存储在 `~/.moltbot/moltbot.json` 的 `tools.web.search.apiKey` 下。

**环境变量替代方案：** 在 Gateway 网关进程环境中设置 `BRAVE_API_KEY`。对于 Gateway 网关安装，将其放在 `~/.moltbot/.env`（或你的服务环境）中。参见[环境变量](/help/faq#how-does-moltbot-load-environment-variables)。
...
**环境变量替代方案：** 在 Gateway 网关环境中设置 `OPENROUTER_API_KEY` 或 `PERPLEXITY_API_KEY`。对于 Gateway 网关安装，将其放在 `~/.moltbot/.env` 中。

如果未设置基础 URL，MoltBot 会根据 API 密钥来源选择默认值：
