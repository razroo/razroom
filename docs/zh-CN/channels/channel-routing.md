...
MoltBot 将回复**路由回消息来源的渠道**。模型不会选择渠道；路由是确定性的，由主机配置控制。
...
广播组允许你为同一对端运行**多个智能体**，**在 MoltBot 正常回复时**触发（例如：在 WhatsApp 群组中，经过提及/激活门控之后）。
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.moltbot/workspace-support" }],
  },
  bindings: [
