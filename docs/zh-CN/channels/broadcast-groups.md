...
广播群组在渠道白名单和群组激活规则之后进行评估。在 WhatsApp 群组中，这意味着广播会在 MoltBot 正常回复时发生（例如：被提及时，具体取决于你的群组设置）。
...
**结果：** 当 MoltBot 在此聊天中回复时，将运行所有三个智能体。
...
Workspace: /Users/pascal/moltbot-alfred/
...
Workspace: /Users/pascal/moltbot-baerbel/
Tools: read only
```
...
```bash
tail -f ~/.moltbot/logs/gateway.log | grep broadcast
```
