...
你可以在 `~/.razroom/razroom.json` 中覆盖此设置：
...
所有日志配置都在 `~/.razroom/razroom.json` 的 `logging` 下。
...
- Razroom 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `razroom plugins enable diagnostics-otel` 启用插件。
...
- `razroom.tokens`（计数器，属性：`razroom.token`、`razroom.channel`、`razroom.provider`、`razroom.model`）
- `razroom.cost.usd`（计数器，属性：`razroom.channel`、`razroom.provider`、`razroom.model`）
- `razroom.run.duration_ms`（直方图，属性：`razroom.channel`、`razroom.provider`、`razroom.model`）
- `razroom.context.tokens`（直方图，属性：`razroom.context`、`razroom.channel`、`razroom.provider`、`razroom.model`）

消息流：

- `razroom.webhook.received`（计数器，属性：`razroom.channel`、`razroom.webhook`）
- `razroom.webhook.error`（计数器，属性：`razroom.channel`、`razroom.webhook`）
- `razroom.webhook.duration_ms`（直方图，属性：`razroom.channel`、`razroom.webhook`）
- `razroom.message.queued`（计数器，属性：`razroom.channel`、`razroom.source`）
- `razroom.message.processed`（计数器，属性：`razroom.channel`、`razroom.outcome`）
- `razroom.message.duration_ms`（直方图，属性：`razroom.channel`、`razroom.outcome`）

队列 + 会话：

- `razroom.queue.lane.enqueue`（计数器，属性：`razroom.lane`）
- `razroom.queue.lane.dequeue`（计数器，属性：`razroom.lane`）
- `razroom.queue.depth`（直方图，属性：`razroom.lane` 或 `razroom.channel=heartbeat`）
- `razroom.queue.wait_ms`（直方图，属性：`razroom.lane`）
- `razroom.session.state`（计数器，属性：`razroom.state`、`razroom.reason`）
- `razroom.session.stuck`（计数器，属性：`razroom.state`）
- `razroom.session.stuck_age_ms`（直方图，属性：`razroom.state`）
- `razroom.run.attempt`（计数器，属性：`razroom.attempt`）
...
- `razroom.webhook.error`
  - `razroom.channel`、`razroom.webhook`、`razroom.chatId`、`razroom.error`
- `razroom.message.processed`
  - `razroom.channel`、`razroom.outcome`、`razroom.chatId`、`razroom.messageId`、`razroom.sessionKey`、`razroom.sessionId`、`razroom.reason`
- `razroom.session.stuck`
  - `razroom.state`、`razroom.ageMs`、`razroom.queueDepth`、`razroom.sessionKey`、`razroom.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `razroom doctor`。
