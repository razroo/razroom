...
- [ ] 更新 CLI/版本字符串：[`src/cli/program.ts`](https://github.com/razroom/razroom/blob/main/src/cli/program.ts) 和 [`src/provider-web.ts`](https://github.com/razroom/razroom/blob/main/src/provider-web.ts) 中的 Baileys user agent。
- [ ] 确认包元数据（name、description、repository、keywords、license）以及 `bin` 映射指向 [`razroom.mjs`](https://github.com/razroom/razroom/blob/main/razroom.mjs) 作为 `razroom`。
- [ ] 如果依赖项有变化，运行 `pnpm install` 确保 `pnpm-lock.yaml` 是最新的。

2. **构建和产物**

- [ ] 如果 A2UI 输入有变化，运行 `pnpm canvas:a2ui:bundle` 并提交更新后的 [`src/canvas-host/a2ui/a2ui.bundle.js`](https://github.com/razroom/razroom/blob/main/src/canvas-host/a2ui/a2ui.bundle.js)。
...
- [ ] `RAZROOM_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`（Docker 安装冒烟测试，快速路径；发布前必需）
  - 如果已知上一个 npm 发布版本有问题，为预安装步骤设置 `RAZROOM_INSTALL_SMOKE_PREVIOUS=<last-good-version>` 或 `RAZROOM_INSTALL_SMOKE_SKIP_PREVIOUS=1`。
- [ ]（可选）完整安装程序冒烟测试（添加非 root + CLI 覆盖）：`pnpm test:install:smoke`
- [ ]（可选）安装程序 E2E（Docker，运行 `curl -fsSL https://razroom.ai/install.sh | bash`，新手引导，然后运行真实工具调用）：
...
- [ ] 生成 Sparkle appcast（通过 [`scripts/make_appcast.sh`](https://github.com/razroom/razroom/blob/main/scripts/make_appcast.sh) 生成 HTML 注释）并更新 `appcast.xml`。
...
- [ ] 按照 [macOS 发布](/platforms/mac/release) 获取确切命令和所需环境变量。
  ...
  - 如果进行公证，使用从 App Store Connect API 环境变量创建的 `razroom-notary` 钥匙串配置文件（参见 [macOS 发布](/platforms/mac/release)）。
...
- **npm pack/publish 挂起或产生巨大 tarball**：`dist/Razroom.app` 中的 macOS 应用包（和发布 zip）被扫入包中。通过 `package.json` 的 `files` 白名单发布内容来修复（包含 dist 子目录、docs、skills；排除应用包）。用 `npm pack --dry-run` 确认 `dist/Razroom.app` 未列出。
...
1. `npm search @razroom --json` 并捕获包名。
