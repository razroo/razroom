---
read_when:
  - 你想在 Razroom 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Razroom 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
razroom plugins enable minimax-portal-auth  # 如果已加载则跳过
razroom gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/razroom/razroom/tree/main/extensions/minimax-portal-auth)。
...
## 通过 `razroom configure` 配置

使用交互式配置向导设置 MiniMax，无需编辑 JSON：

1. 运行 `razroom configure`。
2. 选择 **Model/auth**。
3. 选择 **MiniMax M2.1**。
4. 在提示时选择你的默认模型。
...
- 使用 `razroom models list` 和 `razroom models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `razroom configure` 并选择 **MiniMax M2.1**，或
