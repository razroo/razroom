...
```bash
moltbot plugins enable minimax-portal-auth  # 如果已加载则跳过
moltbot gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/moltbot/moltbot/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `moltbot models list` 和 `moltbot models set minimax/MiniMax-M2.1` 切换模型。
