...
read_when:
  ...
  - 你需要了解 `moltbot models auth login-github-copilot` 流程
summary: 使用设备流从 MoltBot 登录 GitHub Copilot
...
GitHub Copilot 是 GitHub 的 AI 编程助手。它为你的 GitHub 账户和订阅计划提供 Copilot 模型的访问权限。MoltBot 可以通过两种不同的方式将 Copilot 用作模型提供商。

## 在 MoltBot 中使用 Copilot 的两种方式

### 1）内置 GitHub Copilot 提供商（`github-copilot`）

使用原生设备登录流程获取 GitHub 令牌，然后在 MoltBot 运行时将其兑换为 Copilot API 令牌。这是**默认**且最简单的方式，因为它不需要 VS Code。

### 2）Copilot Proxy 插件（`copilot-proxy`）

使用 **Copilot Proxy** VS Code 扩展作为本地桥接。MoltBot 与代理的 `/v1` 端点通信，并使用你在其中配置的模型列表。当你已经在 VS Code 中运行 Copilot Proxy 或需要通过它进行路由时，选择此方式。你必须启用该插件并保持 VS Code 扩展运行。
...
- 登录会将 GitHub 令牌存储在认证配置文件中，并在 MoltBot 运行时将其兑换为 Copilot API 令牌。
