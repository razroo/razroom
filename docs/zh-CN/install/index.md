...
read_when:
  - 安装 MoltBot
  - 你想从 GitHub 安装
summary: 安装 MoltBot（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 moltbot、node-llama-cpp、sharp 等
pnpm add -g moltbot@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm moltbot ...` 运行仓库命令。
...
- 运行新手引导：`moltbot onboard --install-daemon`
- 快速检查：`moltbot doctor`
- 检查 Gateway 网关健康状态：`moltbot status` + `moltbot health`
...
```bash
# 显式 npm
curl -fsSL https://moltbot.ai/install.sh | bash -s -- --install-method npm
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `moltbot`）。
