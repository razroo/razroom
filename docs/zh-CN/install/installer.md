---
read_when:
  - 你想了解 `moltbot.ai/install.sh` 的工作机制
  - 你想自动化安装（CI / 无头环境）
  - 你想从 GitHub 检出安装
...
MoltBot 提供两个安装器脚本（托管在 `moltbot.ai`）：

- `https://moltbot.ai/install.sh` — "推荐"安装器（默认全局 npm 安装；也可从 GitHub 检出安装）
- `https://moltbot.ai/install-cli.sh` — 无需 root 权限的 CLI 安装器（安装到带有独立 Node 的前缀目录）
- `https://moltbot.ai/install.ps1` — Windows PowerShell 安装器（默认 npm；可选 git 安装）

查看当前参数/行为，运行：

```bash
curl -fsSL https://moltbot.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) 帮助：

```powershell
& ([scriptblock]::Create((iwr -useb https://moltbot.ai/install.ps1))) -?
```

如果安装器完成但在新终端中找不到 `moltbot`，通常是 Node/npm PATH 问题。参见：[安装](/install#nodejs--npm-path-sanity)。
...
- 如果是升级现有安装：运行 `moltbot doctor --non-interactive`（尽力执行）。
- 对于 git 安装：安装/更新后运行 `moltbot doctor --non-interactive`（尽力执行）。
- 通过默认设置 `SHARP_IGNORE_GLOBAL_LIBVIPS=1` 来缓解 `sharp` 原生安装问题（避免使用系统 libvips 编译）。

如果你*希望* `sharp` 链接到全局安装的 libvips（或你正在调试），请设置：

```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://moltbot.ai/install.sh | bash
```

### 可发现性 / "git 安装"提示

如果你在**已有的 MoltBot 源码检出目录中**运行安装器（通过 `package.json` + `pnpm-workspace.yaml` 检测），它会提示：

- 更新并使用此检出（`git`）
- 或迁移到全局 npm 安装（`npm`）

在非交互式上下文中（无 TTY / `--no-prompt`），你必须传入 `--install-method git|npm`（或设置 `MOLTBOT_INSTALL_METHOD`），否则脚本将以退出码 `2` 退出。
...
此脚本将 `moltbot` 安装到前缀目录（默认：`~/.moltbot`），同时在该前缀下安装专用的 Node 运行时，因此可以在不想改动系统 Node/npm 的机器上使用。

帮助：

```bash
curl -fsSL https://moltbot.ai/install-cli.sh | bash -s -- --help
```
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g moltbot@latest`
  - `git`：克隆/构建源码检出并安装包装脚本
- 在升级和 git 安装时运行 `moltbot doctor --non-interactive`（尽力执行）。
...
```powershell
iwr -useb https://moltbot.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://moltbot.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\moltbot"
```
