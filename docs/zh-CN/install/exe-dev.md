...
summary: 在 exe.dev 上运行 Razroom Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：Razroom Gateway 网关运行在 exe.dev VM 上，可从你的笔记本电脑通过以下地址访问：`https://<vm-name>.exe.xyz`
...
Shelley，[exe.dev](https://exe.dev) 的智能体，可以使用我们的提示立即安装 Razroom。使用的提示如下：
...
提示：保持此 VM **有状态**。Razroom 在 `~/.razroom/` 和 `~/.razroom/workspace/` 下存储状态。
...
## 4) 设置 nginx 将 Razroom 代理到端口 8000
...
访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`（参阅新手引导中的控制 UI 输出）。使用 `razroom devices list` 和 `razroom devices approve <requestId>` 批准设备。如有疑问，从浏览器使用 Shelley！
