---
read_when:
  - 你正在将 MoltBot 迁移到新的笔记本电脑/服务器
  - 你想保留会话、认证和渠道登录（WhatsApp 等）
summary: 将 MoltBot 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
本指南将 MoltBot Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。

迁移在概念上很简单：

- 复制**状态目录**（`$MOLTBOT_STATE_DIR`，默认：`~/.moltbot/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.moltbot/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- `--profile <name>`（通常变成 `~/.moltbot-<profile>/`）
...
在输出中查找 `MOLTBOT_STATE_DIR` / profile 的提及。如果你运行多个 Gateway 网关，对每个配置文件重复此操作。

### 2）确定你的工作区

常见默认值：

- `~/.moltbot/workspace/`（推荐的工作区）
- 你创建的自定义文件夹
...
tar -czf moltbot-workspace.tgz .moltbot/workspace
```

如果你有多个配置文件/状态目录（例如 `~/.moltbot-main`、`~/.moltbot-work`），分别归档每个。
...
在这个阶段，如果新手引导创建了一个新的 `~/.moltbot/` 也没关系 — 你将在下一步覆盖它。
...
如果你在旧 Gateway 网关上使用了配置文件（或 `MOLTBOT_STATE_DIR`），而新 Gateway 网关使用了不同的配置，你会看到如下症状：
...
`$MOLTBOT_STATE_DIR` 包含密钥（API 密钥、OAuth 令牌、WhatsApp 凭证）。将备份视为生产密钥：
...
- `moltbot status` 显示 Gateway 网关正在运行
...
- [MoltBot 在哪里存储数据？](/help/faq#where-does-moltbot-store-its-data)
