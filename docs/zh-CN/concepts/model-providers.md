...
- CLI 辅助工具：`moltbot onboard`、`moltbot models list`、`moltbot models set <provider/model>`。
...
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  ...
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `moltbot.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。
