---
summary: "Use MiniMax M2.1 in Razroom"
read_when:
  - You want MiniMax models in Razroom
...
```bash
razroom plugins enable minimax-portal-auth  # skip if already loaded.
razroom gateway restart  # restart if gateway is already running
razroom onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/razroom/razroom/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `razroom.json`:
...
- Use `razroom models list` and `razroom models set minimax/MiniMax-M2.1` to switch.
...
- Running `razroom configure` and selecting **MiniMax M2.1**, or
