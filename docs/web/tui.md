...
```bash
razroom tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `razroom tui --deliver`
...
- Check the Gateway logs: `razroom logs --follow`.
- Confirm the agent can run: `razroom status` and `razroom models status`.
...
- No agents in picker: check `razroom agents list` and your routing config.
