...
- [Quick start and first-run setup]
  ...
  - [I can't access docs.moltbot.ai (SSL error). What now?](#i-cant-access-docsmoltbotai-ssl-error-what-now)
  ...
  - [Can I ask MoltBot to update itself?](#can-i-ask-moltbot-to-update-itself)
  ...
  - [Can multiple people use one WhatsApp number with different MoltBot instances?](#can-multiple-people-use-one-whatsapp-number-with-different-moltbot-instances)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two MoltBot instances talk to each other (local + VPS)?](#can-two-moltbot-instances-talk-to-each-other-local-vps)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of MoltBot instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-moltbot-instances-one-ceo-and-many-agents)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do MoltBot, Flawd, and Krill use for models?](#what-do-moltbot-flawd-and-krill-use-for-models)
...
[https://github.com/moltbot/moltbot/issues](https://github.com/moltbot/moltbot/issues)
[https://github.com/moltbot/moltbot/pulls](https://github.com/moltbot/moltbot/pulls)
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `MOLTBOT_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `moltbot config get gateway.auth.token` (or generate one: `moltbot doctor --generate-gateway-token`).
...
3. Copy your workspace (default: `~/.moltbot/workspace`).
...
[https://github.com/moltbot/moltbot/blob/main/CHANGELOG.md](https://github.com/moltbot/moltbot/blob/main/CHANGELOG.md)

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docs.moltbot.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.moltbot.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.moltbot.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsmoltbotai-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

If you still can't reach the site, the docs are mirrored on GitHub:
[https://github.com/moltbot/moltbot/tree/main/docs](https://github.com/moltbot/moltbot/tree/main/docs)
...
[https://moltbot.ai/install.ps1](https://moltbot.ai/install.ps1)
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://moltbot.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```
...
### Can I ask MoltBot to update itself
...
MoltBot supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but MoltBot authorization uses numeric IDs only.
...
### Can multiple people use one WhatsApp number with different MoltBot instances
...
(`~/.moltbot`) and workspace (`~/.moltbot/workspace`) stay untouched.
...
Showcase: [https://moltbot.ai/showcase](https://moltbot.ai/showcase)
...
Yes. Add extra directories via `skills.load.extraDirs` in `~/.moltbot/moltbot.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.moltbot/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which MoltBot treats as `<workspace>/skills`.
...
Not directly. macOS skills are gated by `metadata.moltbot.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
...
3. Override the skill metadata (workspace or `~/.moltbot/skills`) to allow Linux:
   ...
   metadata: { "moltbot": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured MoltBot workspace); MoltBot treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.moltbot/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
...
- Persist `/home/node` with `MOLTBOT_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `MOLTBOT_DOCKER_APT_PACKAGES`.
...
Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.moltbot/workspace`).
...
Default workspace is `~/.moltbot/workspace`, configurable via:
...
If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.moltbot/workspace`).
...
### Can two MoltBot instances talk to each other local VPS
...
### Is there a way to make a team of MoltBot instances one CEO and many agents
...
Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, MoltBot currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `moltbot configure --section model` (interactive)
...
### What do MoltBot, Flawd, and Krill use for models

- **MoltBot + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
...
- Fastest: `moltbot dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [MoltBot assistant setup](/start/moltbot) and [Agent send](/tools/agent-send).
