...
When `requireMention: true` is set for a group chat, MoltBot now transcribes audio **before** checking for mentions. This allows voice notes to be processed even when they contain mentions.

**How it works:**

1. If a voice message has no text body and the group requires mentions, MoltBot performs a "preflight" transcription.
