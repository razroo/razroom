---
summary: "Razroom on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Razroom on a Raspberry Pi
  - Running Razroom on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Razroom on Raspberry Pi

## Goal

Run a persistent, always-on Razroom Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Razroom features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled razroom

# Enable if not
sudo systemctl enable razroom

# Start on boot
sudo systemctl start razroom
```
...
```bash
# Check logs
journalctl -u razroom --no-pager -n 100

# Common fix: rebuild
cd ~/razroom  # if using hackable install
npm run build
sudo systemctl restart razroom
```
