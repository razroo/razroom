...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`MOLTBOT_PROFILE`; legacy `com.moltbot.*` is supported).
...
- Writes `~/.moltbot/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.moltbot/disable-launchagent`,
