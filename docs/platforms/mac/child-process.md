...
service via the external `razroom` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`RAZROOM_PROFILE`; legacy `com.razroom.*` is supported).
