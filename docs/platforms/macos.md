---
summary: "Razroom macOS companion app (menu bar + gateway broker)"
read_when:
...
# Razroom macOS Companion (menu bar + gateway broker)

The macOS app is the **menu‑bar companion** for Razroom. It owns permissions,
...
- Installs the global CLI (`razroom`) via npm/pnpm on request (bun not recommended for the Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `razroom gateway install`.
...
(or `bot.molt.<profile>` when using `--profile`/`RAZROOM_PROFILE`; legacy `com.razroom.*` still unloads).
...
The app registers the `razroom://` URL scheme for local actions.
...
1. Install and launch **Razroom.app**.
...
Tip: compare against `razroom gateway discover --json` to see whether the
