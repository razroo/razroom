...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `moltbot.internal.`), **or**
...
The Gateway advertises `_moltbot-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `moltbot.internal.`) and Tailscale split DNS.
...
```bash
moltbot nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__moltbot__/canvas/"}'
```
