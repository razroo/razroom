...
Matrix is an open, decentralized messaging protocol. Razroom connects as a Matrix **user**
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Razroom calls the same
     login endpoint, stores the access token in `~/.razroom/credentials/matrix/credentials.json`,
...
- On first connection, Razroom requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Razroom logs a warning.
...
`~/.razroom/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, Razroom resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
