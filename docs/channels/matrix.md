...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: MoltBot calls the same
     login endpoint, stores the access token in `~/.moltbot/credentials/matrix/credentials.json`,
...
`~/.moltbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- On startup, MoltBot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
