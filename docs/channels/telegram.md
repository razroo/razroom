...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `moltbot doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
...
```
    ...
    - or read `chat.id` from `moltbot logs --follow`
...
- DM messages can carry `message_thread_id`; MoltBot routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    MoltBot can stream partial replies by sending a temporary Telegram message and editing it as text arrives.
    ...
    For text-only replies, MoltBot keeps the same preview message and performs a final edit in place (no second message).

    For complex replies (for example media payloads), MoltBot falls back to normal final delivery and then cleans up the preview message.

    `streamMode` is separate from block streaming. When block streaming is explicitly enabled for Telegram, MoltBot skips the preview stream to avoid double-streaming.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, MoltBot retries as plain text.
...
```
    ...
    - `~/.moltbot/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, MoltBot enqueues system events like:
...
moltbot message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `moltbot channels status` warns when config expects unmentioned group messages.
    - `moltbot channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `moltbot logs --follow` for skip reasons
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `open` requires `"*"`. `moltbot doctor --fix` can resolve legacy `@username` entries to IDs.
- `channels.telegram.groupPolicy`: `open | allowlist | disabled` (default: allowlist).
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `moltbot doctor --fix` can resolve legacy `@username` entries to IDs.
