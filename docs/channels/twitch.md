...
read_when:
  - Setting up Twitch chat integration for Razroom
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Razroom connects as a Twitch user (bot account) to receive and send messages in channels.
...
4. Configure the token:
   - Env: `RAZROOM_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "razroom", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use RAZROOM_TWITCH_ACCESS_TOKEN env var)
