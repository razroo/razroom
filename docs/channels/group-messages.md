...
- Manual smoke:
  - Send an `@moltbot` ping in the group and confirm a reply that references the sender name.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.moltbot/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
