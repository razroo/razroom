...
- **gateway host** → `razroom` process on the gateway machine
...
`~/.razroom/exec-approvals.json` directly.

CLI: `razroom approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
