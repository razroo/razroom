...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `razroom sandbox explain`.
...
Legacy `agent.*` configs are migrated by `razroom doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=razroom-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${RAZROOM_STATE_DIR:-$HOME/.razroom}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
