...
`~/.moltbot/moltbot.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.moltbot/.env` (or your
...
environment. For a gateway install, put it in `~/.moltbot/.env`.
