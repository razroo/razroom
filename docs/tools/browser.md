...
read_when:
  ...
  - Debugging why moltbot is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (MoltBot-managed)"
---

# Browser (moltbot-managed)
...
- The `moltbot` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The default `chrome` profile uses the **system default Chromium browser** via the
  extension relay; switch to `moltbot` for the isolated managed browser.

## What you get

- A separate browser profile named **moltbot** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`moltbot`, `work`, `remote`, ...).
...
## Profiles: `moltbot` vs `chrome`

- `moltbot`: managed, isolated browser (no extension required).
- `chrome`: extension relay to your **system browser** (requires the MoltBot
  extension to be attached to a tab).

Set `browser.defaultProfile: "moltbot"` if you want managed mode by default.
...
- Default profile is `chrome` (extension relay). Use `defaultProfile: "moltbot"` for the managed browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `moltbot` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
- If browser control is enabled and no auth is configured, MoltBot auto-generates `gateway.auth.token` on startup and persists it to config.
...
- **moltbot-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **extension relay**: your existing Chrome tab(s) via the local relay + Chrome extension

Defaults:

- The `moltbot` profile is auto-created if missing.
...
MoltBot can also drive **your existing Chrome tabs** (no separate “moltbot” Chrome instance) via a local CDP relay + a Chrome extension.
...
- `x-moltbot-password: <gateway password>` or HTTP Basic auth with that password

### Playwright requirement

Some features (navigate/act/AI snapshot/role snapshot, element screenshots, PDF) require
Playwright. If Playwright isn’t installed, those endpoints return a clear 501
error. ARIA snapshots and basic screenshots still work for moltbot-managed Chrome.
...
`MOLTBOT_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `moltbot browser download e12 report.pdf`
- `moltbot browser waitfordownload report.pdf`
- `moltbot browser upload /tmp/moltbot/uploads/file.pdf`
...
- Download and trace output paths are constrained to MoltBot temp roots:
  - traces: `/tmp/moltbot` (fallback: `${os.tmpdir()}/moltbot`)
  - downloads: `/tmp/moltbot/downloads` (fallback: `${os.tmpdir()}/moltbot/downloads`)
- Upload paths are constrained to an MoltBot temp uploads root:
  - uploads: `/tmp/moltbot/uploads` (fallback: `${os.tmpdir()}/moltbot/uploads`)
- `upload` can also set file inputs directly via `--input-ref` or `--element`.
- `snapshot`:
  - `--format ai` (default when Playwright is installed): returns an AI snapshot with numeric refs (`aria-ref="<n>"`).
  - `--format aria`: returns the accessibility tree (no refs; inspection only).
  - `--efficient` (or `--mode efficient`): compact role snapshot preset (interactive + compact + depth + lower maxChars).
  - Config default (tool/CLI only): set `browser.snapshotDefaults.mode: "efficient"` to use efficient snapshots when the caller does not pass a mode (see [Gateway configuration](/gateway/configuration#browser-moltbot-managed-browser)).
...
- The moltbot browser profile may contain logged-in sessions; treat it as sensitive.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (moltbot, chrome, or remote CDP).
