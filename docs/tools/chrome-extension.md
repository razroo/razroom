---
summary: "Chrome extension: let Razroom drive your existing Chrome tab"
read_when:
...
The Razroom Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate razroom-managed Chrome profile.
...
Razroom then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Razroom release (npm package) as static files. There is no separate “build” step.

After upgrading Razroom:

- Re-run `razroom browser extension install` to refresh the installed files under your Razroom state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Razroom ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Razroom to control.
...
- `ON`: attached; Razroom can drive that tab.
...
Debugging: `razroom sandbox explain`
...
`razroom browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `razroom browser extension install` first to copy the extension to a stable location under your Razroom state directory.
...
- **This is not isolated** like the dedicated razroom-managed profile.
