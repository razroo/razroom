...
- A public registry for MoltBot skills.
...
4. Users browse, download, and install skills in MoltBot.
...
If you want to add new capabilities to your MoltBot agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
By default, the CLI installs skills into `./skills` under your current working directory. If a MoltBot workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). MoltBot loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.moltbot/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches MoltBot how to perform a
...
Interested in becoming a moderator? Ask in the MoltBot Discord and contact a
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/moltbot/skills` and `~/.moltbot/skills`). This is designed to find older skill installs without extra flags.
