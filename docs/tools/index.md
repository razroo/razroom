---
summary: "Agent tool surface for Razroom (browser, canvas, nodes, message, cron) replacing legacy `razroom-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `razroom-*` skills
title: "Tools"
---

# Tools (Razroom)

Razroom exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `razroom-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `razroom.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Razroom logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`razroom node run`).
...
- Requires a Brave API key (recommended: `razroom configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Razroom-managed browser.
...
- Quick smoke: `razroom nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `razroom gateway` restart in-place)
