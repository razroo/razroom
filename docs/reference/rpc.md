...
Razroom integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Razroom owns lifecycle when `channels.signal.autoStart=true`.
...
- Razroom spawns `imsg rpc` as a child process (legacy iMessage integration).
