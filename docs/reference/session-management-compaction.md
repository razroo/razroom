...
This document explains how Razroom manages sessions end-to-end:
...
Razroom is designed around a single **Gateway process** that owns session state.
...
Razroom persists sessions in two layers:
...
- Transcripts: `~/.razroom/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Razroom resolves these via `src/config/sessions.ts`.
...
Razroom intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Razroom consumes the events, but Pi decides when to compact).
...
Razroom also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Razroom bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Razroom leaves it alone.
...
- `razroom status` (CLI)
- `razroom sessions` / `sessions --json`
...
Razroom supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Razroom strips/suppresses this in the delivery layer.

As of `2026.1.10`, Razroom also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Razroom uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Razroom’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `razroom status`.
