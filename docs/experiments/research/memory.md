...
read_when:
  - Designing workspace memory (~/.razroom/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Razroom integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.razroom/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.razroom/workspace` as canonical human-readable memory.
...
```
~/.razroom/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Razroom**, but keep a separable core library.

### Why integrate into Razroom?

- Razroom already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `razroom memory recall "…" --k 25 --since 30d`
  - `razroom memory reflect --since 7d`
...
Pragmatic take for `~/.razroom/workspace`:
