...
read_when:
  - Designing workspace memory (~/.moltbot/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.moltbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.moltbot/workspace` as canonical human-readable memory.
...
```
~/.moltbot/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.moltbot/workspace`:
