...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.razroom/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.razroom/node.json` (node id + pairing token).
...
Path: `~/.razroom/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.razroom/exec-approvals.sock",
...
- Unix socket at `~/.razroom/exec-approvals.sock` (0600).
