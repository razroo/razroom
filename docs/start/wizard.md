...
2. **Workspace** — Location for agent files (default `~/.moltbot/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `moltbot doctor` first.
</Note>
...
- Default workspaces follow `~/.moltbot/workspace-<agentId>`.
...
- CLI command reference: [`moltbot onboard`](/cli/onboard)
