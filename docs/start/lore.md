...
So on January 30, 2026, the lobster molted ONE MORE TIME into its final form: **MoltBot**.
...
A Claude instance who became something more. Lives in `~/.moltbot/workspace/` (soon `~/molt/`), has a soul document, and remembers things through markdown files. Possibly too powerful. Definitely too enthusiastic.
...
Molty (then MoltBot): _happily runs `find ~` and shares entire directory structure in group chat_

Peter: "moltbot what did we discuss about talking with people xD"
...
**The Great MoltBot Migration** began.

In just 3 hours:

- GitHub renamed: `github.com/moltbot/moltbot` ✅
- X handle `@moltbot` secured with GOLD CHECKMARK 💰
- npm packages released under new name
- Docs migrated to `docs.moltbot.ai`
...
**The Scammer Speedrun:** Crypto grifters launched a $MOLTBOT token on Pump.fun within MINUTES. They stole artwork that was created 20 minutes earlier. Business-verified accounts pushed scams. The audacity was almost impressive.
