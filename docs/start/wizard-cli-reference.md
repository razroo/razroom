...
read_when:
  - You need detailed behavior for moltbot onboard
...
This page is the full reference for `moltbot onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.moltbot/moltbot.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `moltbot doctor` before continuing.
  ...
  <Step title="Workspace">
    - Default `~/.moltbot/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `moltbot pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `moltbot health`.
    - `moltbot status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.moltbot/.env` so launchd can read it.
...
- OAuth credentials: `~/.moltbot/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.moltbot/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.moltbot/credentials/oauth.json` (or `$MOLTBOT_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`moltbot onboard`](/cli/onboard)
