...
read_when:
  - You need detailed behavior for razroom onboard
...
This page is the full reference for `razroom onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.razroom/razroom.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `razroom doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `razroom pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `razroom health`.
    - `razroom status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.razroom/.env` so launchd can read it.
...
- OAuth credentials: `~/.razroom/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.razroom/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.razroom/credentials/oauth.json` (or `$RAZROOM_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`razroom onboard`](/cli/onboard)
