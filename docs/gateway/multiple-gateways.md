...
- `MOLTBOT_CONFIG_PATH` — per-instance config file
- `MOLTBOT_STATE_DIR` — per-instance sessions, creds, caches
...
Profiles auto-scope `MOLTBOT_STATE_DIR` + `MOLTBOT_CONFIG_PATH` and suffix service names.
...
Base port = `gateway.port` (or `MOLTBOT_GATEWAY_PORT` / `--port`).
