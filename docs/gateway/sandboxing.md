---
summary: "How Razroom sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Razroom can run **tools inside Docker containers** to reduce blast radius.
...
- `"none"` (default): tools see a sandbox workspace under `~/.razroom/sandboxes`.
- `"ro"`: mounts the agent workspace read-only at `/agent` (disables `write`/`edit`/`apply_patch`).
- `"rw"`: mounts the agent workspace read/write at `/workspace`.

Inbound media is copied into the active sandbox workspace (`media/inbound/*`).
Skills note: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`,
Razroom mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
Default image: `razroom-sandbox:bookworm-slim`
...
- Use `razroom sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
