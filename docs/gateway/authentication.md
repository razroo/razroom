...
Razroom supports OAuth and API keys for model providers. For Anthropic
...
2. Put it on the **gateway host** (the machine running `razroom gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.razroom/.env` so the daemon can read it:
...
API keys for daemon use: `razroom onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.razroom/.env`, systemd/launchd).
...
Run `razroom models status` to confirm which profile is expiring. If the profile
