...
- Canvas host: `/__moltbot__/canvas/` and `/__moltbot__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
- Prefer a dedicated profile for the agent (the default `moltbot` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only MoltBot clients can connect.
...
1. Email: [security@moltbot.ai](mailto:security@moltbot.ai)
