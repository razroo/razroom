...
## Quick check: `razroom security audit`
...
```bash
razroom security audit
razroom security audit --deep
razroom security audit --fix
```
...
- Tighten local perms (`~/.razroom` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

Razroom is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Razroom also attempts a best-effort live Gateway probe.
...
`razroom security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${RAZROOM_GATEWAY_PASSWORD}
```
...
Razroom stores session transcripts on disk under `~/.razroom/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.razroom` (see the audit section below). If you need
...
Razroom can refresh the skills list mid-session:
...
- If you install plugins from npm (`razroom plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.razroom/extensions/<pluginId>/` (or `$RAZROOM_STATE_DIR/extensions/<pluginId>/`).
  - Razroom uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
razroom pairing approve <channel> <code>
```
...
By default, Razroom routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Razroom has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.razroom/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.razroom or your logs.”
...
4. **Re-run audit**
   - `razroom security audit --deep` and confirm the report is clean.
...
- `~/.razroom/razroom.json`: `600` (user read/write only)
- `~/.razroom`: `700` (user only)

`razroom doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `RAZROOM_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__razroom__/canvas/` and `/__razroom__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_razroom-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `RAZROOM_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `razroom doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `RAZROOM_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `RAZROOM_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Razroom
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. Razroom verifies the identity by resolving the
...
- Razroom will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.razroom/` (or `$RAZROOM_STATE_DIR/`) may contain secrets or private data:

- `razroom.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `razroom status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@razroom", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.razroom/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `razroom` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only Razroom clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `razroom gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `RAZROOM_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/razroom/razroom-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.razroom/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + Razroom version
...
Found a vulnerability in Razroom? Please report responsibly:

1. Email: [security@razroom.ai](mailto:security@razroom.ai)
