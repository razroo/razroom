...
- During heartbeat runs, Razroom treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `razroom sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Razroom skips the heartbeat run entirely (no model call).
...
```bash
razroom system event --text "Check for urgent follow-ups" --mode now
```
