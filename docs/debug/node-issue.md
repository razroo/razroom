...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Razroom
...
Running Razroom via Node with `tsx` fails at startup with:

```
[razroom] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `razroom status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch razroom.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node razroom.mjs status` works on Node 25.
