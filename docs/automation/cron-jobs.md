...
Cron jobs are persisted on the Gateway host at `~/.moltbot/cron/jobs.json` by default.
...
If `delivery` is omitted for isolated jobs, MoltBot defaults to `announce`.
...
- Check cron is enabled: `cron.enabled` and `MOLTBOT_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- MoltBot applies exponential retry backoff for recurring jobs after consecutive errors:
