...
| `internal_port = 3000`         | Must match `--port 3000` (or `MOLTBOT_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `MOLTBOT_STATE_DIR = "/data"` | Persists state on the volume                                                |
...
- Non-loopback binds (`--bind lan`) require `MOLTBOT_GATEWAY_TOKEN` for security.
...
**Note:** With `MOLTBOT_STATE_DIR=/data`, the config path is `/data/moltbot.json`.
...
Paste your gateway token (the one from `MOLTBOT_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `MOLTBOT_GATEWAY_PORT=3000`).
...
**Fix:** Ensure `MOLTBOT_STATE_DIR=/data` is set in `fly.toml` and redeploy.
