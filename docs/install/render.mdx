...
Deploy Razroom on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/razroom/razroom)
...
repository configures everything needed to run Razroom:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: RAZROOM_STATE_DIR
        value: /data/.razroom
      - key: RAZROOM_WORKSPACE_DIR
        value: /data/workspace
      - key: RAZROOM_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/razroom`.
...
If you use the original Razroom repository, Render will not auto-deploy your Razroom. To update it, run a manual Blueprint sync from the dashboard.
...
For Razroom, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Razroom host.
