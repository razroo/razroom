...
- `/usage cost` in chats: local cost summary aggregated from Razroom session logs.
- CLI: `razroom status --usage` prints a full per-provider breakdown.
- CLI: `razroom channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
