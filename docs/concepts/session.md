...
- You can verify your DM settings with `moltbot security audit` (see [security](/cli/security)).
...
- On the **gateway host**:
  - Store file: `~/.moltbot/agents/<agentId>/sessions/sessions.json` (per agent).
- Transcripts: `~/.moltbot/agents/<agentId>/sessions/<SessionId>.jsonl` (Telegram topic sessions use `.../<SessionId>-topic-<threadId>.jsonl`).
