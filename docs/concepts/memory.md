...
`~/.moltbot/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.moltbot/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.moltbot/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, MoltBot retries with `qmd query`. If QMD fails or the binary is
  missing, MoltBot automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- MoltBot does not expose QMD embed batch-size tuning today; batch behavior is
...
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  ...
  - If you want to pre-download models manually (and warm the same index MoltBot
    ...
    MoltBot’s QMD state lives under your **state dir** (defaults to `~/.moltbot`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    MoltBot uses:

    ```bash
    # Pick the same state dir MoltBot uses
    STATE_DIR="${MOLTBOT_STATE_DIR:-$HOME/.moltbot}"
...
- When `scope` denies a search, MoltBot logs a warning with the derived
...
- When `memory.qmd.sessions.enabled = true`, MoltBot exports sanitized session
  ...
  `~/.moltbot/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- Index storage: per-agent SQLite at `~/.moltbot/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
...
- Session logs live on disk (`~/.moltbot/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
