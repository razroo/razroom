...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    razroomHome: process.env.RAZROOM_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("RAZROOM_HOME", snapshot.razroomHome);
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked RAZROOM_HOME.
  delete process.env.RAZROOM_HOME;
  process.env.RAZROOM_STATE_DIR = path.join(base, ".razroom");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "razroom-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".razroom", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
