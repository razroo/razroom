...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    moltbotHome: process.env.MOLTBOT_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MOLTBOT_HOME", snapshot.moltbotHome);
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MOLTBOT_HOME.
  delete process.env.MOLTBOT_HOME;
  process.env.MOLTBOT_STATE_DIR = path.join(base, ".moltbot");
...
export async function withTempHome<T>(
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".moltbot", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
