...
process.env.MOLTBOT_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime MoltBot modules.
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: MoltBotConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
