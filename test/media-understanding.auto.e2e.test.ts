...
import type { RazroomConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("razroom-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("razroom-bin-sherpa-");
      const modelDir = await makeTempDir("razroom-sherpa-model-");
      ...
      const cfg: RazroomConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("razroom-bin-whispercpp-");
      const modelDir = await makeTempDir("razroom-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("razroom-bin-gemini-");
      ...
      const cfg: RazroomConfig = { tools: { media: { image: {} } } };
