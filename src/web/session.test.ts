...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    ...
    const makeWASocket = baileys.makeWASocket as ReturnType<typeof mock>;
  ...
  it("logWebSelfId prints cached E.164 when creds exist", () => {
    const existsSpy = spyOn(fsSync, "existsSync").mockImplementation((p) => {
      if (typeof p !== "string") {
        return false;
      }
      return p.endsWith("creds.json");
    });
    const readSpy = spyOn(fsSync, "readFileSync").mockImplementation((p) => {
  ...
  it("does not clobber creds backup when creds.json is corrupted", async () => {
    const credsSuffix = path.join(".razroom", "credentials", "whatsapp", "default", "creds.json");

    const copySpy = spyOn(fsSync, "copyFileSync").mockImplementation(() => {});
    const existsSpy = spyOn(fsSync, "existsSync").mockImplementation((p) => {
      if (typeof p !== "string") {
        return false;
      }
    ...
    const statSpy = spyOn(fsSync, "statSync").mockImplementation((p) => {
