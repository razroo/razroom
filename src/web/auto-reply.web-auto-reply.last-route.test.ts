...
function makeCfg(storePath: string): RazroomConfig {
  return {
...
function createHandlerForTest(opts: { cfg: RazroomConfig; replyResolver: unknown }) {
...
describe("web auto-reply last-route", () => {
  ...
  it("updates last-route for direct chats without senderE164", async () => {
    ...
    const replyResolver = mock().mockResolvedValue(undefined);
    ...
    await handler({
      ...
      chatType: "direct",
      chatId: "direct:+1000",
      sendComposing: mock().mockResolvedValue(undefined),
      reply: mock().mockResolvedValue(undefined),
      sendMedia: mock().mockResolvedValue(undefined),
    });
