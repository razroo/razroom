...
describe("isBotMentionedFromTargets", () => {
  const mentionCfg = { mentionRegexes: [/\brazroom\b/i] };

  it("ignores regex matches when other mentions are present", () => {
    const msg = makeMsg({
      body: "@Razroom please help",
  ...
  it("ignores JID mentions in self-chat mode", () => {
    const cfg = { mentionRegexes: [/\brazroom\b/i], allowFrom: ["+999"] };
    const msg = makeMsg({
      body: "@owner ping",
...
describe("resolveMentionTargets with @lid mapping", () => {
  it("resolves mentionedJids via lid reverse mapping in authDir", async () => {
    const authDir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-lid-mapping-"));
    try {
      await fs.writeFile(
