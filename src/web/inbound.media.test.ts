...
const readAllowFromStoreMock = mock().mockResolvedValue([]);
const upsertPairingRequestMock = mock().mockResolvedValue({ code: "PAIRCODE", created: true });
const saveMediaBufferSpy = mock();
...
mock("../media/store.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../media/store.js")>();
  return {
    ...actual,
    saveMediaBuffer: mock(async (...args: Parameters<typeof actual.saveMediaBuffer>) => {
...
mock("@whiskeysockets/baileys", async () => {
  ...
  return {
    ...actual,
    downloadMediaMessage: mock().mockResolvedValue(jpegBuffer),
  };
});
...
mock("./session.js", () => {
  ...
  const sock = {
    ev,
    ws: { close: mock() },
    sendPresenceUpdate: mock().mockResolvedValue(undefined),
    sendMessage: mock().mockResolvedValue(undefined),
    readMessages: mock().mockResolvedValue(undefined),
    updateMediaMessage: mock(),
    logger: {},
  ...
  return {
    ...
    getStatusCode: mock(() => 200),
  };
});

import { monitorWebInbox, resetWebInboundDedupe } from "./inbound.js";

async function waitForMessage(onMessage: ReturnType<typeof mock>) {
