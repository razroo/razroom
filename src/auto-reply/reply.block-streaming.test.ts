...
const piEmbeddedMock = vi.hoisted(() => ({
  ...
  runEmbeddedPiAgent: mock<ReturnType<RunEmbeddedPiAgent>, Parameters<RunEmbeddedPiAgent>>(),
...
mock("/src/agents/pi-embedded.js", () => piEmbeddedMock);
mock("../agents/pi-embedded.js", () => piEmbeddedMock);
...
describe("block streaming", () => {
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-stream-"));
  });
  ...
  it("handles ordering, timeout fallback, and telegram streamMode block", async () => {
    await withTempHome(async (home) => {
      ...
      const seen: string[] = [];
      const onBlockReply = mock(async (payload) => {
      ...
      const onBlockReplyStreamMode = mock().mockResolvedValue(undefined);
