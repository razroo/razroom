...
import { scheduleGatewaySigusr1Restart, triggerRazroomRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Razroom in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerRazroomRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Razroom via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
