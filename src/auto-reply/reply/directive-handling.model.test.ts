...
mock("../../agents/agent-scope.js", () => ({
  resolveAgentConfig: mock(() => ({})),
  resolveAgentDir: mock(() => "/tmp/agent"),
  resolveSessionAgentId: mock(() => "main"),
}));

mock("../../agents/sandbox.js", () => ({
  resolveSandboxRuntimeStatus: mock(() => ({ sandboxed: false })),
}));

mock("../../config/sessions.js", () => ({
  updateSessionStore: mock(async () => {}),
}));
...
function baseConfig(): RazroomConfig {
  return {
    commands: { text: true },
    agents: { defaults: {} },
...
describe("/model chat UX", () => {
  it("shows summary for /model with no args", async () => {
    ...
    const cfg = { commands: { text: true } } as unknown as RazroomConfig;
