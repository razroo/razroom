...
mock("../../agents/pi-embedded.js", () => ({
  abortEmbeddedPiRun: mock().mockReturnValue(true),
  resolveEmbeddedSessionLane: (key: string) => `session:${key.trim() || "main"}`,
}));

const commandQueueMocks = vi.hoisted(() => ({
  clearCommandLane: mock(),
}));

mock("../../process/command-queue.js", () => commandQueueMocks);

const subagentRegistryMocks = vi.hoisted(() => ({
  listSubagentRunsForRequester: mock(() => []),
  markSubagentRunTerminated: mock(() => 1),
}));

mock("../../agents/subagent-registry.js", () => ({
...
describe("abort detection", () => {
  ...
  it("triggerBodyNormalized extracts /stop from RawBody for abort detection", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-abort-"));
  ...
  it("fast-aborts even when text commands are disabled", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-abort-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath }, commands: { text: false } } as RazroomConfig;
