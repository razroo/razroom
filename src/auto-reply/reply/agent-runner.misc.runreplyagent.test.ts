...
const runCliAgentMock = mock();
const runWithModelFallbackMock = mock();
const runtimeErrorMock = mock();
...
mock("../../agents/pi-embedded.js", async () => {
...
mock("../../agents/cli-runner.js", async () => {
...
mock("./queue.js", async () => {
  ...
  return {
    ...actual,
    enqueueFollowupRun: mock(),
    scheduleFollowupDrain: mock(),
  };
});
...
describe("runReplyAgent claude-cli routing", () => {
  ...
  it("uses claude-cli runner for claude-cli provider", async () => {
    const randomSpy = spyOn(crypto, "randomUUID").mockReturnValue("run-1");
