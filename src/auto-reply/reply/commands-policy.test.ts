...
const validateConfigObjectWithPluginsMock = vi.hoisted(() => mock());
const writeConfigFileMock = vi.hoisted(() => mock());
...
const readChannelAllowFromStoreMock = vi.hoisted(() => mock());
const addChannelAllowFromStoreEntryMock = vi.hoisted(() => mock());
const removeChannelAllowFromStoreEntryMock = vi.hoisted(() => mock());

mock("../../pairing/pairing-store.js", async () => {
...
mock("../../channels/plugins/pairing.js", async () => {
...
function buildParams(commandBody: string, cfg: RazroomConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes Slack DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    const written = writeConfigFileMock.mock.calls[0]?.[0] as RazroomConfig;
