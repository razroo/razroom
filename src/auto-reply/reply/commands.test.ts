...
mock("./commands-context-report.js", () => ({
...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-commands-"));
...
describe("handleCommands hooks", () => {
  it("triggers hooks for /new with arguments", async () => {
    ...
    const spy = spyOn(internalHooks, "triggerInternalHook").mockResolvedValue();
