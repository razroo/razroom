...
describe("resolveSlackMedia", () => {
  beforeEach(() => {
    mockFetch = mock();
    globalThis.fetch = mockFetch as typeof fetch;
    spyOn(ssrf, "resolvePinnedHostname").mockImplementation(async (hostname) => {
  ...
  it("prefers url_private_download over url_private", async () => {
    spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue({
      path: "/tmp/test.jpg",
  ...
  it("overrides video/* MIME to audio/* for slack_audio voice messages", async () => {
    ...
    const saveMediaBufferMock = spyOn(mediaStore, "saveMediaBuffer").mockResolvedValue({
      path: "/tmp/voice.mp4",
  ...
  it("returns all successfully downloaded files as an array", async () => {
    spyOn(mediaStore, "saveMediaBuffer").mockImplementation(async (buffer) => {
...
describe("resolveSlackThreadHistory", () => {
  ...
  it("includes file-only messages and drops empty-only entries", async () => {
    const replies = mock().mockResolvedValueOnce({
      messages: [
  ...
  it("returns empty when Slack API throws", async () => {
    const replies = mock().mockRejectedValueOnce(new Error("slack down"));
    const client = {
      conversations: { replies },
    } as Parameters<typeof resolveSlackThreadHistory>[0]["client"];
