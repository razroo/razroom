...
function createHarness(overrides?: {
  ...
  const postEphemeral = mock().mockResolvedValue({ ok: true });
  const app = {
...
}): Promise<{ respond: ReturnType<typeof mock>; ack: ReturnType<typeof mock> }> {
  const handler = [...params.commands.values()][0];
  if (!handler) {
    throw new Error("Missing slash handler");
  }

  const respond = mock().mockResolvedValue(undefined);
  const ack = mock().mockResolvedValue(undefined);
