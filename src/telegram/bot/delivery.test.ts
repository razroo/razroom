...
describe("deliverReplies", () => {
  ...
  it("invokes onVoiceRecording before sending a voice note", async () => {
    ...
    const onVoiceRecording = mock(async () => {
  ...
  it("renders markdown in media captions", async () => {
    const runtime = { error: mock(), log: mock() };
    const sendPhoto = mock().mockResolvedValue({
      message_id: 2,
      chat: { id: "123" },
    });
  ...
  it("rethrows non-VOICE_MESSAGES_FORBIDDEN errors from sendVoice", async () => {
    const runtime = { error: mock(), log: mock() };
    const sendVoice = mock().mockRejectedValue(new Error("Network error"));
