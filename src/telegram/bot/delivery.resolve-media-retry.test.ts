...
const saveMediaBuffer = mock();
const fetchRemoteMedia = mock();
...
describe("resolveMedia getFile retry", () => {
  ...
  it("returns null when all getFile retries fail so message is not dropped", async () => {
    const getFile = mock().mockRejectedValue(new Error("Network request for 'getFile' failed!"));
  ...
  it("does not catch errors from fetchRemoteMedia (only getFile is retried)", async () => {
    const getFile = mock().mockResolvedValue({ file_path: "voice/file_0.oga" });
  ...
  it("returns null for photo when getFile exhausts retries", async () => {
    const getFile = mock().mockRejectedValue(new Error("HttpError: Network error"));
