...
describe("sendMessageTelegram", () => {
  ...
  it("uses native fetch for BAN compatibility when api is omitted", async () => {
    ...
    const fetchSpy = mock() as unknown as typeof fetch;
  ...
  it("wraps chat-not-found with actionable context", async () => {
    ...
    const sendMessage = mock().mockRejectedValue(err);
    const api = { sendMessage } as unknown as {
      sendMessage: typeof sendMessage;
    };
  ...
  it("does not retry on non-transient errors", async () => {
    const chatId = "123";
    const sendMessage = mock().mockRejectedValue(new Error("400: Bad Request"));
    const api = { sendMessage } as unknown as {
      sendMessage: typeof sendMessage;
    };
  ...
  it("sends GIF media as animation", async () => {
    const chatId = "123";
    const sendAnimation = mock().mockResolvedValue({
      message_id: 9,
      chat: { id: chatId },
    });
  ...
  it("sends audio media as files by default", async () => {
    const chatId = "123";
    const sendAudio = mock().mockResolvedValue({
      message_id: 10,
      chat: { id: chatId },
    });
    const sendVoice = mock().mockResolvedValue({
      message_id: 11,
      chat: { id: chatId },
    });
  ...
  it("does not retry thread-not-found when no message_thread_id was provided", async () => {
    const chatId = "123";
    const threadErr = new Error("400: Bad Request: message thread not found");
    const sendMessage = mock().mockRejectedValueOnce(threadErr);
    const api = { sendMessage } as unknown as {
      sendMessage: typeof sendMessage;
    };
...
describe("sendStickerTelegram", () => {
  ...
  it("sends a sticker by file_id", async () => {
    ...
    const sendSticker = mock().mockResolvedValue({
      message_id: 100,
      chat: { id: chatId },
    });
  ...
  it("wraps chat-not-found with actionable context", async () => {
    const chatId = "123";
    const err = new Error("400: Bad Request: chat not found");
    const sendSticker = mock().mockRejectedValue(err);
    const api = { sendSticker } as unknown as {
      sendSticker: typeof sendSticker;
    };
