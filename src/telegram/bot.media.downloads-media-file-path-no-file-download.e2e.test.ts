...
const cacheStickerSpy = mock();
const getCachedStickerSpy = mock();
const describeStickerImageSpy = mock();
const resolvePinnedHostname = ssrf.resolvePinnedHostname;
const lookupMock = mock();
let resolvePinnedHostnameSpy: ReturnType<typeof spyOn> = null;
...
async function createBotHandler(): Promise<{
  ...
  runtimeError: ReturnType<typeof mock>;
}> {
...
function mockTelegramFileDownload(params: {
  ...
  return spyOn(globalThis, "fetch" as never).mockResolvedValueOnce({
    ok: true,
    status: 200,
    statusText: "OK",
...
describe("telegram inbound media", () => {
  ...
  it("prefers proxyFetch over global fetch", async () => {
    ...
    const runtimeError = mock();
    const globalFetchSpy = spyOn(globalThis, "fetch" as never).mockImplementation(() => {
      throw new Error("global fetch should not be called");
    });
    const proxyFetch = mock().mockResolvedValueOnce({
      ok: true,
      status: 200,
      statusText: "OK",
...
describe("telegram media groups", () => {
  ...
  it(
    ...
    async () => {
      ...
      const runtimeError = mock();
      const fetchSpy = spyOn(globalThis, "fetch" as never).mockResolvedValue({
        ok: true,
        status: 200,
        statusText: "OK",
...
describe("telegram stickers", () => {
  ...
  it(
    ...
    async () => {
      ...
      const fetchSpy = spyOn(globalThis, "fetch" as never).mockResolvedValueOnce({
        ok: true,
        status: 200,
        statusText: "OK",
