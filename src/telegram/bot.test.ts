...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/razroom-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
...
mock("../config/sessions.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../config/sessions.js")>();
  return {
    ...actual,
    resolveStorePath: mock((storePath) => storePath ?? sessionStorePath),
  };
});

const { readChannelAllowFromStore, upsertChannelPairingRequest } = vi.hoisted(() => ({
  readChannelAllowFromStore: mock(async () => [] as string[]),
  upsertChannelPairingRequest: mock(async () => ({
    code: "PAIRCODE",
    created: true,
  })),
}));
...
const { wasSentByBot } = vi.hoisted(() => ({
  wasSentByBot: mock(() => false),
}));
mock("./sent-message-cache.js", () => ({
  wasSentByBot,
  recordSentMessage: mock(),
  clearSentMessageCache: mock(),
}));

const useSpy = mock();
const middlewareUseSpy = mock();
const onSpy = mock();
const stopSpy = mock();
const commandSpy = mock();
const botCtorSpy = mock();
const answerCallbackQuerySpy = mock(async () => undefined);
const sendChatActionSpy = mock();
const editMessageTextSpy = mock(async () => ({ message_id: 88 }));
const setMessageReactionSpy = mock(async () => undefined);
const setMyCommandsSpy = mock(async () => undefined);
const sendMessageSpy = mock(async () => ({ message_id: 77 }));
const sendAnimationSpy = mock(async () => ({ message_id: 78 }));
const sendPhotoSpy = mock(async () => ({ message_id: 79 }));
type ApiStub = {
...
mock("grammy", () => ({
  ...
  InputFile: class {},
  webhookCallback: mock(),
}));

const sequentializeMiddleware = mock();
const sequentializeSpy = mock(() => sequentializeMiddleware);
...
const throttlerSpy = mock(() => "throttler");

mock("@grammyjs/transformer-throttler", () => ({
  apiThrottler: () => throttlerSpy(),
}));

mock("../auto-reply/reply.js", () => {
  const replySpy = mock(async (_ctx, opts) => {
...
describe("createTelegramBot", () => {
  ...
  it("blocks callback_query when inline buttons are allowlist-only and sender not authorized", async () => {
    onSpy.mockReset();
    const replySpy = replyModule.__replySpy as unknown as ReturnType<typeof mock>;
    replySpy.mockReset();
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Razroom" },
        },
      },
      me: { id: 999, username: "razroom_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
