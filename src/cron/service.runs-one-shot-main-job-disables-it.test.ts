...
const fixturesRoot = abs(path.join("__razroom_vitest__", "cron", "runs-one-shot"));
...
describe("CronService", () => {
  ...
  it("passes agentId to runHeartbeatOnce for main-session wakeMode now jobs", async () => {
    ...
    const enqueueSystemEvent = mock();
    const requestHeartbeatNow = mock();
    const runHeartbeatOnce = mock(async () => ({ status: "ran" as const, durationMs: 1 }));
  ...
  it("wakeMode now falls back to queued heartbeat when main lane stays busy", async () => {
    ...
    const enqueueSystemEvent = mock();
    const requestHeartbeatNow = mock();
    const runHeartbeatOnce = mock(async () => ({
