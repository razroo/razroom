...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "razroom-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".razroom", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<RazroomConfig> = {},
): RazroomConfig {
  const base: RazroomConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".razroom", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
