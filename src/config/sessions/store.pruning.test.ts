...
describe("rotateSessionFile", () => {
  ...
  it("multiple rotations: only keeps 3 most recent .bak files", async () => {
    let now = Date.now();
    const nowSpy = spyOn(Date, "now").mockImplementation(() => (now += 5));
    try {
      for (let i = 0; i < 5; i++) {
...
// The file-level mock("../config.js") stubs loadConfig; per-test
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    mockLoadConfig = configModule.loadConfig as ReturnType<typeof mock>;
  });
