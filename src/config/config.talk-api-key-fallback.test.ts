...
describe("talk api key fallback", () => {
  it("reads ELEVENLABS_API_KEY from profile when env is missing", () => {
    const existsSync = mock((candidate: string) => candidate.endsWith(".profile"));
    const readFileSync = mock(() => "export ELEVENLABS_API_KEY=profile-key\n");
    const homedir = mock(() => "/tmp/home");
