...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "moltbot-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-moltbot",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-moltbot#general");
  });
  ...
  it("derives session transcripts dir from MOLTBOT_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MOLTBOT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.MOLTBOT_STATE_DIR = "/custom/state";
    try {
