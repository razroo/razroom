...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-razroom",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-razroom#general");
  });
  ...
  it("derives session transcripts dir from RAZROOM_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { RAZROOM_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.RAZROOM_STATE_DIR = "/custom/state";
    try {
