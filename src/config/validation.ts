...
import type { RazroomConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: RazroomConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: RazroomConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = RazroomSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as RazroomConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as RazroomConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as RazroomConfig,
  };
}
