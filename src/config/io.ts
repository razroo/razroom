...
import type { RazroomConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareRazroomVersions } from "./version.js";
...
function coerceConfig(value: unknown): RazroomConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as RazroomConfig;
}
...
function stampConfigVersion(cfg: RazroomConfig): RazroomConfig {
...
function warnIfConfigFromFuture(cfg: RazroomConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareRazroomVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Razroom (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as RazroomConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): RazroomConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as RazroomConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: RazroomConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as RazroomConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.RAZROOM_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.RAZROOM_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.RAZROOM_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.RAZROOM_WATCH_SESSION === "string" &&
        deps.env.RAZROOM_WATCH_SESSION.trim().length > 0
          ? deps.env.RAZROOM_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.RAZROOM_WATCH_COMMAND === "string" &&
        deps.env.RAZROOM_WATCH_COMMAND.trim().length > 0
          ? deps.env.RAZROOM_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `RAZROOM_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.RAZROOM_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.RAZROOM_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): RazroomConfig {
