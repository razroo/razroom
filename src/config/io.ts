...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as MoltBotConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  async function writeConfigFile(cfg: MoltBotConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as MoltBotConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.MOLTBOT_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.MOLTBOT_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.MOLTBOT_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.MOLTBOT_WATCH_SESSION === "string" &&
        deps.env.MOLTBOT_WATCH_SESSION.trim().length > 0
          ? deps.env.MOLTBOT_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.MOLTBOT_WATCH_COMMAND === "string" &&
        deps.env.MOLTBOT_WATCH_COMMAND.trim().length > 0
          ? deps.env.MOLTBOT_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.MOLTBOT_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
