...
describe("tailscale helpers", () => {
  ...
  it("ensureGoInstalled installs when missing and user agrees", async () => {
    const exec = mock().mockRejectedValueOnce(new Error("no go")).mockResolvedValue({}); // brew install go
    const prompt = mock().mockResolvedValue(true);
    const runtime = {
      error: mock(),
      log: mock(),
      exit: ((code: number) => {
  ...
  it("ensureTailscaledInstalled installs when missing and user agrees", async () => {
    const exec = mock().mockRejectedValueOnce(new Error("missing")).mockResolvedValue({});
    const prompt = mock().mockResolvedValue(true);
    const runtime = {
      error: mock(),
      log: mock(),
  ...
  it("enableTailscaleServe does NOT use sudo if first attempt succeeds", async () => {
    const exec = mock().mockResolvedValue({ stdout: "" });
  ...
  it("ensureFunnel uses fallback for enabling", async () => {
    ...
    const runtime = {
      error: mock(),
      log: mock(),
      exit: mock() as unknown as (code: number) => never,
    };
    const prompt = mock();
  ...
  it("enableTailscaleServe skips sudo on non-permission errors", async () => {
    const exec = mock().mockRejectedValueOnce(new Error("boom"));
