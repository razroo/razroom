...
const mocks = vi.hoisted(() => ({
  createService: mock(),
  shutdown: mock(),
  registerUnhandledRejectionHandler: mock(),
  logWarn: mock(),
  logDebug: mock(),
}));
...
describe("gateway bonjour advertiser", () => {
  ...
  beforeEach(() => {
    spyOn(logging, "getLogger").mockReturnValue({
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    const advertise = mock().mockImplementation(
      async () =>
    ...
    expect(gatewayCall?.[0]?.type).toBe("razroom-gw");
  ...
  it("omits cliPath and sshPort in minimal mode", async () => {
    ...
    const destroy = mock().mockResolvedValue(undefined);
    const advertise = mock().mockResolvedValue(undefined);
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    spyOn(os, "hostname").mockReturnValue("Mac.localdomain");
    ...
    expect(gatewayCall?.[0]?.name).toBe("razroom (Razroom)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("razroom");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("razroom.local");
