...
describe("ssrf pinning", () => {
  ...
  it("rejects private DNS results", async () => {
    const lookup = mock(async () => [{ address: "10.0.0.8", family: 4 }]);
  ...
  it("falls back for non-matching hostnames", async () => {
    const fallback = mock((host: string, options?: unknown, callback?: unknown) => {
  ...
  it("enforces hostname allowlist when configured", async () => {
    const lookup = mock(async () => [{ address: "93.184.216.34", family: 4 }]);
