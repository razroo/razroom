...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.RAZROOM_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*razroom-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "razroom";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "razroom");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "razroom-gw");
  ...
  records.push(`_razroom-gw._tcp IN PTR ${instanceLabel}._razroom-gw._tcp`);
  records.push(`${instanceLabel}._razroom-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._razroom-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; razroom-content-hash: ${contentHash}\n${contentBody}`;
}
