...
const hookMocks = vi.hoisted(() => ({
  runner: {
    hasHooks: mock(() => false),
    runMessageSent: mock(async () => {}),
  },
}));
const queueMocks = vi.hoisted(() => ({
  enqueueDelivery: mock(async () => "mock-queue-id"),
  ackDelivery: mock(async () => {}),
  failDelivery: mock(async () => {}),
}));
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks telegram markdown and passes through accountId", async () => {
    const sendTelegram = mock().mockResolvedValue({ messageId: "m1", chatId: "c1" });
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = mock().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: RazroomConfig = { channels: { signal: { mediaMaxMb: 2 } } };
  ...
  it("respects newline chunk mode for WhatsApp", async () => {
    const sendWhatsApp = mock().mockResolvedValue({ messageId: "w1", toJid: "jid" });
    const cfg: RazroomConfig = {
  ...
  it("preserves fenced blocks for markdown chunkers in newline mode", async () => {
    const chunker = mock((text: string) => (text ? [text] : []));
    const sendText = mock().mockImplementation(async ({ text }: { text: string }) => ({
      channel: "matrix" as const,
      messageId: text,
      roomId: "r1",
    }));
    const sendMedia = mock().mockImplementation(async ({ text }: { text: string }) => ({
      channel: "matrix" as const,
      messageId: text,
      roomId: "r1",
    }));
  ...
  it("uses iMessage media maxBytes from agent fallback", async () => {
    const sendIMessage = mock().mockResolvedValue({ messageId: "i1" });
    setActivePluginRegistry(
      createTestRegistry([
        {
          pluginId: "imessage",
  ...
  it("passes normalized payload to onError", async () => {
    const sendWhatsApp = mock().mockRejectedValue(new Error("boom"));
    const onError = mock();
    const cfg: RazroomConfig = {};
  ...
  it("emits message_sent success for sendPayload deliveries", async () => {
    hookMocks.runner.hasHooks.mockImplementation((name: string) => name === "message_sent");
    const sendPayload = mock().mockResolvedValue({ channel: "matrix", messageId: "mx-1" });
    const sendText = mock();
  ...
  it("emits message_sent failure when delivery errors", async () => {
    hookMocks.runner.hasHooks.mockImplementation((name: string) => name === "message_sent");
    const sendWhatsApp = mock().mockRejectedValue(new Error("downstream failed"));
