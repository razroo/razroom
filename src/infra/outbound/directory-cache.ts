...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: RazroomConfig | null = null;
  ...
  get(key: string, cfg: RazroomConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: RazroomConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: RazroomConfig): void {
