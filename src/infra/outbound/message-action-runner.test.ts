...
describe("runMessageAction media caption behavior", () => {
  ...
  it("promotes caption to message for media sends when message is empty", async () => {
    const sendMedia = mock().mockResolvedValue({
      channel: "testchat",
      messageId: "m1",
      chatId: "c1",
    });
      ...
      createTestRegistry([
        {
          ...
          plugin: createOutboundTestPlugin({
            ...
            outbound: {
              deliveryMode: "direct",
              sendText: mock().mockResolvedValue({
                channel: "testchat",
                messageId: "t1",
                chatId: "c1",
              }),
...
describe("runMessageAction card-only send behavior", () => {
  const handleAction = mock(async ({ params }: { params: Record<string, unknown> }) =>
    jsonResult({
      ok: true,
...
describe("runMessageAction accountId defaults", () => {
  const handleAction = mock(async () => jsonResult({ ok: true }));
