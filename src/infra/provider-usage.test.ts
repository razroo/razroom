...
describe("provider usage loading", () => {
  it("loads usage snapshots with injected auth", async () => {
    ...
    const mockFetch = mock<Parameters<typeof fetch>, ReturnType<typeof fetch>>(async (input) => {
      const url =
  ...
  it("discovers Claude usage from token auth profiles", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        const mockFetch = mock<Parameters<typeof fetch>, ReturnType<typeof fetch>>(
          async (input, init) => {
            const url =
              typeof input === "string"
                ? input
      ...
      {
        env: {
          RAZROOM_STATE_DIR: (home) => path.join(home, ".razroom"),
        },
        prefix: "razroom-provider-usage-",
      },
    );
  });
