...
const mockPrimary = {
  search: mock(async () => []),
  readFile: mock(async () => ({ text: "", path: "MEMORY.md" })),
  status: mock(() => ({
  ...
  sync: mock(async () => {}),
  probeEmbeddingAvailability: mock(async () => ({ ok: true })),
  probeVectorAvailability: mock(async () => true),
  close: mock(async () => {}),
};

const fallbackSearch = mock(async () => [
  {
    path: "MEMORY.md",
    startLine: 1,
    endLine: 1,
...
const mockMemoryIndexGet = mock(async () => fallbackManager);

mock("./qmd-manager.js", () => ({
  QmdMemoryManager: {
    create: mock(async () => mockPrimary),
  },
}));
