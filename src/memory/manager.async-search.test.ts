...
const embedQuery = mock(async () => [0.2, 0.2, 0.2]);
...
describe("memory search async sync", () => {
  ...
  beforeEach(async () => {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-mem-async-"));
  ...
  it("does not await sync when searching", async () => {
    ...
    (manager as unknown as { sync: () => Promise<void> }).sync = mock(async () => pending);
