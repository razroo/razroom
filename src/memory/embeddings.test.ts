...
const importNodeLlamaCppMock = mock();
mock("./node-llama.js", () => ({
...
describe("local embedding normalization", () => {
  ...
  it("normalizes local embeddings to magnitude ~1.0", async () => {
    ...
    const resolveModelFileMock = mock(async () => "/fake/model.gguf");

    importNodeLlamaCppMock.mockResolvedValue({
      getLlama: async () => ({
        loadModel: mock().mockResolvedValue({
          createEmbeddingContext: mock().mockResolvedValue({
            getEmbeddingFor: mock().mockResolvedValue({
