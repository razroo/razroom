...
let runtimeStub: {
  ...
  manager: {
    initiateCall: ReturnType<typeof mock>;
    continueCall: ReturnType<typeof mock>;
    speak: ReturnType<typeof mock>;
    endCall: ReturnType<typeof mock>;
    getCall: ReturnType<typeof mock>;
    getCallByProviderCallId: ReturnType<typeof mock>;
  };
  stop: ReturnType<typeof mock>;
};

mock("../../extensions/voice-call/src/runtime.js", () => ({
  createVoiceCallRuntime: mock(async () => runtimeStub),
}));
...
function setup(config: Record<string, unknown>): Registered {
  ...
  plugin.register({
    ...
    config: {},
    pluginConfig: config,
    runtime: { tts: { textToSpeechTelephony: mock() } },
    logger: noopLogger,
...
describe("voice-call plugin", () => {
  beforeEach(() => {
    runtimeStub = {
      ...
      manager: {
        initiateCall: mock(async () => ({ callId: "call-1", success: true })),
        continueCall: mock(async () => ({
          success: true,
          transcript: "hello",
        })),
        speak: mock(async () => ({ success: true })),
        endCall: mock(async () => ({ success: true })),
        getCall: mock((id: string) => (id === "call-1" ? { callId: "call-1" } : undefined)),
        getCallByProviderCallId: mock(() => undefined),
      },
      stop: mock(async () => {}),
    };
  });
