...
mock("@mariozechner/pi-ai", () => ({
  ...
  getOAuthProviders: () => [],
  getOAuthApiKey: mock(async () => null),
}));

mock("../agents/pi-embedded-runner/model.js", () => ({
  resolveModel: mock((provider: string, modelId: string) => ({
    model: {
      provider,
      id: modelId,
      name: modelId,
...
mock("../agents/model-auth.js", () => ({
  getApiKeyForModel: mock(async () => ({
    apiKey: "test-api-key",
    source: "test",
    mode: "api-key",
  })),
  requireApiKey: mock((auth: { apiKey?: string }) => auth.apiKey ?? ""),
}));
...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    it("skips auto-TTS when inbound audio gating is on and the message is not audio", async () => {
      const prevPrefs = process.env.RAZROOM_TTS_PREFS;
      process.env.RAZROOM_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      process.env.RAZROOM_TTS_PREFS = prevPrefs;
    });
