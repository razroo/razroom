...
describe("deliverLineAutoReply", () => {
  function createDeps(overrides?: Partial<LineAutoReplyDeps>) {
    const replyMessageLine = mock(async () => ({}));
    const pushMessageLine = mock(async () => ({}));
    const pushTextMessageWithQuickReplies = mock(async () => ({}));
    const createTextMessageWithQuickReplies = mock((text: string) => ({
      type: "text" as const,
      text,
    }));
    const createQuickReplyItems = mock((labels: string[]) => ({ items: labels }));
    const pushMessagesLine = mock(async () => ({ messageId: "push", chatId: "u1" }));
  ...
  it("uses reply token for rich-only payloads", async () => {
    ...
    const { deps, replyMessageLine, pushMessagesLine, createQuickReplyItems } = createDeps({
      ...
      chunkMarkdownText: () => [],
      sendLineReplyChunks: mock(async () => ({ replyTokenUsed: false })),
    });
  ...
  it("sends rich messages before quick-reply text so quick replies remain visible", async () => {
    const createTextMessageWithQuickReplies = mock((text: string, _quickReplies: string[]) => ({
      type: "text" as const,
      text,
      quickReply: { items: ["A"] },
    }));
