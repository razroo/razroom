...
const upsertPairingRequestMock = vi.hoisted(() => mock());
const enqueueSystemEventMock = vi.hoisted(() => mock());
...
mock("../../infra/system-events.js", async (importOriginal) => {
...
const createDmButtonInteraction = (overrides: Partial<ButtonInteraction> = {}) => {
  ...
  const defer = mock().mockResolvedValue(undefined);
  const interaction = {
