...
const STORE_PATH = path.join(os.tmpdir(), "razroom-exec-approvals-test.json");
...
const mockRestPost = vi.hoisted(() => mock());
const mockRestPatch = vi.hoisted(() => mock());
const mockRestDelete = vi.hoisted(() => mock());

mock("../send.shared.js", async (importOriginal) => {
...
describe("ExecApprovalButton", () => {
  function createMockHandler(approverIds: string[]) {
    ...
    handler.resolveApproval = mock().mockResolvedValue(true);
    return handler;
  }

  function createMockInteraction(userId: string) {
    const reply = mock().mockResolvedValue(undefined);
    const update = mock().mockResolvedValue(undefined);
    const followUp = mock().mockResolvedValue(undefined);
    const interaction = {
      userId,
      reply,
      update,
  ...
  it("denies unauthorized users with ephemeral message", async () => {
    ...
    // oxlint-disable-next-line typescript/unbound-method -- mock() mock
  ...
  it("follows up with error when resolve fails", async () => {
    const handler = createMockHandler(["111"]);
    handler.resolveApproval = mock().mockResolvedValue(false);
    const ctx: ExecApprovalButtonContext = { handler };
    const button = new ExecApprovalButton(ctx);
