...
mock("../../auto-reply/reply/dispatch-from-config.js", () => ({
  dispatchReplyFromConfig: mock(async () => ({
    queuedFinal: false,
    counts: { final: 0, tool: 0, block: 0 },
  })),
}));

mock("../../auto-reply/reply/reply-dispatcher.js", () => ({
...
async function createBaseContext(overrides: Record<string, unknown> = {}) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-discord-"));
