...
async function main() {
  ...
  const { ensureRazroomCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[razroom] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[razroom] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
