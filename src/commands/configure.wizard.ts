...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.MOLTBOT_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.MOLTBOT_GATEWAY_PASSWORD;
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.moltbot.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
