...
const mocks = vi.hoisted(() => ({
  loadSessionStore: mock().mockReturnValue({
    "+1000": {
  ...
  resolveMainSessionKey: mock().mockReturnValue("agent:main:main"),
  resolveStorePath: mock().mockReturnValue("/tmp/sessions.json"),
  webAuthExists: mock().mockResolvedValue(true),
  getWebAuthAgeMs: mock().mockReturnValue(5000),
  readWebSelfId: mock().mockReturnValue({ e164: "+1999" }),
  logWebSelfId: mock(),
  probeGateway: mock().mockResolvedValue({
    ok: false,
  ...
  callGateway: mock().mockResolvedValue({}),
  listAgentsForGateway: mock().mockReturnValue({
    defaultId: "main",
    mainKey: "agent:main:main",
    scope: "per-sender",
    agents: [{ id: "main", name: "Main" }],
  }),
  runSecurityAudit: mock().mockResolvedValue({
    ts: 0,
...
mock("../memory/manager.js", () => ({
  MemoryIndexManager: {
    get: mock(async ({ agentId }: { agentId: string }) => ({
      probeVectorAvailability: mock(async () => true),
      status: () => ({
        files: 2,
        chunks: 3,
        dirty: false,
...
mock("../config/sessions.js", () => ({
  ...
  resolveFreshSessionTotalTokens: mock(
...
mock("../gateway/probe.js", () => ({
  probeGateway: mocks.probeGateway,
}));
mock("../gateway/call.js", async (importOriginal) => {
...
mock("../infra/razroom-root.js", () => ({
  resolveRazroomPackageRoot: mock().mockResolvedValue("/tmp/razroom"),
}));
...
mock("../infra/update-check.js", () => ({
  checkUpdateStatus: mock().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/razroom/pnpm-lock.yaml",
      markerPath: "/tmp/razroom/node_modules/.modules.yaml",
    },
    registry: { latestVersion: "0.0.0" },
  }),
  compareSemverStrings: mock(() => 0),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Razroom status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("razroom status --all") ||
          l.includes("razroom --profile isolated status --all") ||
          l.includes("razroom status --all") ||
          l.includes("razroom --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.RAZROOM_GATEWAY_TOKEN = "abcd1234";
    try {
