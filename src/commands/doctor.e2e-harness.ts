...
export const resolveRazroomPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadRazroomPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveRazroomPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.RAZROOM_STATE_DIR;
  originalUpdateInProgress = process.env.RAZROOM_UPDATE_IN_PROGRESS;
  process.env.RAZROOM_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "razroom-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.RAZROOM_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.RAZROOM_UPDATE_IN_PROGRESS;
  } else {
    process.env.RAZROOM_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
