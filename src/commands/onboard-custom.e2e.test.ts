...
// Mock dependencies
mock("./model-picker.js", () => ({
  applyPrimaryModel: mock((cfg) => cfg),
}));
...
describe("promptCustomApiConfig", () => {
  ...
  it("handles openai flow and saves alias", async () => {
    const prompter = {
      ...
      select: mock().mockResolvedValueOnce("openai"), // Compatibility
      confirm: mock(),
      note: mock(),
    };
    ...
    const result = await promptCustomApiConfig({
      ...
      runtime: { ...defaultRuntime, log: mock() },
      config: {},
    });
  ...
  it("detects openai compatibility when unknown", async () => {
    const prompter = {
      ...
      select: mock().mockResolvedValueOnce("unknown"),
      confirm: mock(),
      note: mock(),
    };
  ...
  it("re-prompts base url when unknown detection fails", async () => {
    const prompter = {
      ...
      select: mock().mockResolvedValueOnce("unknown").mockResolvedValueOnce("baseUrl"),
      confirm: mock(),
      note: mock(),
    };
  ...
  it("renames provider id when baseUrl differs", async () => {
    const prompter = {
      ...
      select: mock().mockResolvedValueOnce("openai"),
      confirm: mock(),
      note: mock(),
    };
  ...
  it("aborts verification after timeout", async () => {
    ...
    const prompter = {
      ...
      select: mock().mockResolvedValueOnce("openai").mockResolvedValueOnce("model"),
      confirm: mock(),
      note: mock(),
    };
