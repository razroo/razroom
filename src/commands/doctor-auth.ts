...
function pruneAuthProfiles(
  cfg: RazroomConfig,
  profileIds: Set<string>,
): { next: RazroomConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("razroom models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("razroom models auth setup-token")} or ${formatCliCommand(
      "razroom configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "razroom models auth login --provider openai-codex",
    )} or ${formatCliCommand("razroom configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("razroom configure")}\` or \`${formatCliCommand("razroom onboard")}\`.`;
}
