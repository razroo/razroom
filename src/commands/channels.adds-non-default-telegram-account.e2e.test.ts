...
describe("channels command", () => {
  ...
  it("disables a default provider account when remove has no delete flag", async () => {
    ...
    const prompt = { confirm: mock().mockResolvedValue(true) };
  ...
  it("surfaces Discord privileged intent issues in channels status output", () => {
    ...
    expect(lines.join("\n")).toMatch(/Run: (?:razroom|razroom)( --profile isolated)? doctor/);
  });
        ...
        telegram: [
          {
            ...
            enabled: true,
            configured: true,
            probe: { ok: true, bot: { username: "razroom_bot" } },
          },
        ],
      },
    });
    expect(lines.join("\n")).toMatch(/bot:@razroom_bot/);
  });
