...
mock("../config/sessions.js", () => ({
  ...
  updateLastRoute: mock().mockResolvedValue(undefined),
}));

mock("../web/auth-store.js", () => ({
  webAuthExists: mock(async () => true),
  getWebAuthAgeMs: mock(() => 1234),
  readWebSelfId: mock(() => ({ e164: null, jid: null })),
  logWebSelfId: mock(),
  logoutWeb: mock(),
}));
...
describe("getHealthSnapshot", () => {
  ...
  it("treats telegram.tokenFile as configured", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "razroom-health-"));
