...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = mock(async (name: string) =>
      name === "RAZROOM_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("RAZROOM_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv RAZROOM_GATEWAY_TOKEN");
    expect(message).not.toContain("RAZROOM_GATEWAY_PASSWORD");
  });
