...
mock("./shared.js", () => ({
  requireValidConfig: mock(async () => ({ channels: {} })),
  formatChannelAccountLabel: mock(
    ({ channel, accountId }: { channel: string; accountId: string }) => `${channel}:${accountId}`,
  ),
}));

mock("../../channels/plugins/index.js", () => ({
  listChannelPlugins: mock(),
  getChannelPlugin: mock(),
}));

mock("../../slack/scopes.js", () => ({
  fetchSlackScopes: mock(),
}));
...
describe("channelsCapabilitiesCommand", () => {
  ...
  it("prints Slack bot + user scopes when user token is configured", async () => {
    const plugin = buildPlugin({
      ...
      probe: { ok: true, bot: { name: "razroom" }, team: { name: "team" } },
    });
