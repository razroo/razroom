...
mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    loadConfig: mock().mockImplementation(() => mockCfg),
  };
});
...
describe("sandbox explain command", () => {
  it("prints JSON shape + fix-it keys", async () => {
    mockCfg = {
      ...
      session: { store: "/tmp/razroom-test-sessions-{agentId}.json" },
    };
    ...
    expect(parsed).toHaveProperty("docsUrl", "https://docs.razroom.ai/sandbox");
