...
const loadModelCatalog = vi.hoisted(() => mock());
...
const listProfilesForProvider = vi.hoisted(() => mock(() => []));
const upsertAuthProfile = vi.hoisted(() => mock());
const upsertAuthProfileWithLock = vi.hoisted(() => mock(async () => {}));
...
const resolveEnvApiKey = vi.hoisted(() => mock(() => undefined));
const getCustomProviderApiKey = vi.hoisted(() => mock(() => undefined));
...
describe("promptDefaultModel", () => {
  it("filters internal router models from the selection list", async () => {
    ...
    const select = mock(async (params) => {
      const first = params.options[0];
      return first?.value ?? "";
    });
    const prompter = makePrompter({ select });
    const config = { agents: { defaults: {} } } as RazroomConfig;
...
describe("promptModelAllowlist", () => {
  it("filters internal router models from the selection list", async () => {
    ...
    const multiselect = mock(async (params) =>
