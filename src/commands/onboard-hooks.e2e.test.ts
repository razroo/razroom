...
mock("../hooks/hooks-status.js", () => ({
  buildWorkspaceHookStatus: mock(),
}));

mock("../agents/agent-scope.js", () => ({
  resolveAgentWorkspaceDir: mock().mockReturnValue("/mock/workspace"),
  resolveDefaultAgentId: mock().mockReturnValue("main"),
}));
...
describe("onboard-hooks", () => {
  ...
  const createMockPrompter = (multiselectValue: string[]): WizardPrompter => ({
    ...
    note: mock().mockResolvedValue(undefined),
    intro: mock().mockResolvedValue(undefined),
    outro: mock().mockResolvedValue(undefined),
    text: mock().mockResolvedValue(""),
    select: mock().mockResolvedValue(""),
    multiselect: mock().mockResolvedValue(multiselectValue),
    progress: mock().mockReturnValue({
      stop: mock(),
      update: mock(),
    }),
  });
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.razroom/hooks",
    hooks: [
      {
        name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("should show informative notes to user", async () => {
      ...
      const noteCalls = (prompter.note as ReturnType<typeof mock>).mock.calls;
      ...
      expect(noteCalls[1][0]).toMatch(/(?:razroom|razroom)( --profile isolated)? hooks list/);
    });
  });
});
