...
mock("../../llm-slug-generator.js", () => ({
  generateSlugViaLLM: mock().mockResolvedValue("simple-math"),
}));
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => MoltBotConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("moltbot-session-memory-");
