...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "moltbot-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.MOLTBOT_BUNDLED_HOOKS_DIR;
    process.env.MOLTBOT_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.MOLTBOT_BUNDLED_HOOKS_DIR;
    } else {
      process.env.MOLTBOT_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
