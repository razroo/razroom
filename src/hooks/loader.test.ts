...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.RAZROOM_BUNDLED_HOOKS_DIR;
    process.env.RAZROOM_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.RAZROOM_BUNDLED_HOOKS_DIR;
    } else {
      process.env.RAZROOM_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
