...
describe("hooks", () => {
  ...
  describe("getRegisteredEventKeys", () => {
    it("should return all registered event keys", () => {
      registerInternalHook("command:new", mock());
      registerInternalHook("command:stop", mock());
      registerInternalHook("session:start", mock());
  ...
  describe("integration", () => {
    it("should handle a complete hook lifecycle", async () => {
      ...
      const handler = mock((event: InternalHookEvent) => {
        results.push(event);
      });
