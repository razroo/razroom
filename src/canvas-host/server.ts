...
(() => {
  ...
  const hasHelper = () => typeof window.moltbotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("moltbot:a2ui-action-status", onStatus);
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.MOLTBOT_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.MOLTBOT_SKIP_CANVAS_HOST)) {
    return true;
