...
describe("canvas host state dir defaults", () => {
  ...
  it("uses RAZROOM_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Razroom Canvas");
    } finally {
      await handler.close();
