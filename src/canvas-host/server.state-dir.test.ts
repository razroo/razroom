...
describe("canvas host state dir defaults", () => {
  ...
  it("uses MOLTBOT_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "moltbot-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("MoltBot Canvas");
    } finally {
      await handler.close();
