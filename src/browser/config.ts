...
import {
  ...
  DEFAULT_MOLTBOT_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "moltbot" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_MOLTBOT_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "moltbot" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MOLTBOT_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MOLTBOT_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an MoltBot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for moltbot/moltbot CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_MOLTBOT_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "moltbot";
