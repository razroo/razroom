import type { BrowserConfig, BrowserProfileConfig, RazroomConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_RAZROOM_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "razroom" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_RAZROOM_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "razroom" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_RAZROOM_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_RAZROOM_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Razroom browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for razroom/razroom CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: RazroomConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_RAZROOM_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "razroom";
