...
const pwMocks = vi.hoisted(() => ({
  cookiesGetViaPlaywright: mock(async () => ({
    cookies: [{ name: "session", value: "abc123" }],
  })),
  storageGetViaPlaywright: mock(async () => ({ values: { token: "value" } })),
  evaluateViaPlaywright: mock(async () => "ok"),
}));

const routeCtxMocks = vi.hoisted(() => {
  const profileCtx = {
    profile: { cdpUrl: "http://127.0.0.1:9222" },
    ensureTabAvailable: mock(async () => ({
      targetId: "tab-1",
      url: "https://example.com",
    })),
    stopRunningBrowser: mock(async () => {}),
  };

  return {
    profileCtx,
    createBrowserRouteContext: mock(() => ({
      state: () => ({ resolved: { evaluateEnabled: false } }),
      forProfile: mock(() => profileCtx),
      mapTabError: mock(() => null),
    })),
  };
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          razroom: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
    writeConfigFile: mock(async () => {}),
  };
});

mock("./pw-ai-module.js", () => ({
  getPwAiModule: mock(async () => pwMocks),
}));

mock("./server-context.js", async (importOriginal) => {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.RAZROOM_GATEWAY_PORT;
    process.env.RAZROOM_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.RAZROOM_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.RAZROOM_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.RAZROOM_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.RAZROOM_GATEWAY_TOKEN;
    } else {
      process.env.RAZROOM_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.RAZROOM_GATEWAY_PASSWORD;
    } else {
      process.env.RAZROOM_GATEWAY_PASSWORD = prevGatewayPassword;
    }
