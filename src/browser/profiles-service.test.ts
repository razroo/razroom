...
mock("./trash.js", () => ({
  movePathToTrash: mock(async (targetPath: string) => targetPath),
}));

mock("./chrome.js", () => ({
  resolveOpenClawUserDataDir: mock(() => "/tmp/openclaw-test/openclaw/user-data"),
}));
...
function createCtx(resolved: BrowserServerState["resolved"]) {
  ...
  const ctx = {
    ...
    forProfile: mock(() => ({
      stopRunningBrowser: mock(async () => ({ stopped: true })),
    })),
