...
function fakePage(): {
  ...
  mocks: {
    ...
    getByRole: ReturnType<typeof mock>;
    frameLocator: ReturnType<typeof mock>;
    locator: ReturnType<typeof mock>;
  };
} {
  const handlers = new Map<string, Array<(...args: unknown[]) => void>>();
  const on = mock((event: string, cb: (...args: unknown[]) => void) => {
    const list = handlers.get(event) ?? [];
    list.push(cb);
    handlers.set(event, list);
    return undefined as unknown;
  });
  const getByRole = mock(() => ({ nth: mock(() => ({ ok: true })) }));
  const frameLocator = mock(() => ({
    getByRole: mock(() => ({ nth: mock(() => ({ ok: true })) })),
  }));
  const locator = mock(() => ({ nth: mock(() => ({ ok: true })) }));
