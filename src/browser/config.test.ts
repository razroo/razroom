...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(razroom?.driver).toBe("razroom");
    expect(razroom?.cdpPort).toBe(18800);
    expect(razroom?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from RAZROOM_GATEWAY_PORT when unset", () => {
    const prev = process.env.RAZROOM_GATEWAY_PORT;
    process.env.RAZROOM_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const razroom = resolveProfile(resolved, "razroom");
      expect(razroom?.cdpPort).toBe(19012);
      expect(razroom?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.RAZROOM_GATEWAY_PORT;
      } else {
        process.env.RAZROOM_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const razroom = resolveProfile(resolved, "razroom");
      expect(razroom?.cdpPort).toBe(19022);
      expect(razroom?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.RAZROOM_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "razroom");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        razroom: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("razroom");
  });
});
