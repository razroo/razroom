...
describe("pw-tools-core", () => {
  it("last file-chooser arm wins", async () => {
    ...
    const fc1 = { setFiles: mock(async () => {}) };
    const fc2 = { setFiles: mock(async () => {}) };
  ...
  it("arms the next dialog and accepts/dismisses (default timeout)", async () => {
    ...
    const waitForEvent = mock(async () => dialog);
    setPwToolsCoreCurrentPage({
      waitForEvent,
    });
  ...
  it("waits for selector, url, load state, and function", async () => {
    const waitForSelector = mock(async () => {});
    const waitForURL = mock(async () => {});
    const waitForLoadState = mock(async () => {});
    const waitForFunction = mock(async () => {});
    ...
    const page = {
      ...
      waitForFunction,
      waitForTimeout,
      getByText: mock(() => ({ first: () => ({ waitFor: mock() }) })),
    };
