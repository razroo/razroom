...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredRazroomTmpDir: mock(() => "/tmp/razroom"),
}));
mock("../infra/tmp-razroom-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredRazroomTmpDir.mockReturnValue("/tmp/razroom");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    tmpDirMocks.resolvePreferredRazroomTmpDir.mockReturnValue("/tmp/razroom-preferred");
    setPwToolsCoreCurrentPage({ on, off });
    ...
    expect(tmpDirMocks.resolvePreferredRazroomTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "razroom-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "razroom-preferred", "downloads")}${path.sep}`),
    );
  });
