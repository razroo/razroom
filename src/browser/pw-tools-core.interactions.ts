  ...
  try {
    if (boxes.length > 0) {
      await page.evaluate((labels) => {
        const existing = document.querySelectorAll("[data-razroom-labels]");
        existing.forEach((el) => el.remove());

        const root = document.createElement("div");
        root.setAttribute("data-razroom-labels", "1");
        ...
        for (const label of labels) {
          ...
          box.setAttribute("data-razroom-labels", "1");
          ...
          tag.setAttribute("data-razroom-labels", "1");
