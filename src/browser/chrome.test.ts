...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "moltbot-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateMoltBotProfile(userDataDir, { color: DEFAULT_MOLTBOT_BROWSER_COLOR });
    ...
    expect(def.name).toBe(DEFAULT_MOLTBOT_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_MOLTBOT_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".moltbot-profile-decorated"),
      "utf-8",
    );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateMoltBotProfile(userDataDir, { color: "lobster-orange" });
    const localState = await readJson(path.join(userDataDir, "Local State"));
    const profile = localState.profile as Record<string, unknown>;
    const infoCache = profile.info_cache as Record<string, unknown>;
    const def = infoCache.Default as Record<string, unknown>;
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_MOLTBOT_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("picks the first existing Chrome candidate on Windows", () => {
    ...
    const exists = spyOn(fs, "existsSync").mockImplementation((p) => {
  ...
  it("finds Chrome in Program Files on Windows", () => {
    ...
    const exists = spyOn(fs, "existsSync").mockImplementation((p) => String(p).includes(marker));
    const exe = findChromeExecutableWindows();
    expect(exe?.kind).toBe("chrome");
    expect(exe?.path).toMatch(/chrome\.exe$/);
    exists.mockRestore();
  ...
  it("reports reachability based on /json/version", async () => {
    ...
    vi.stubGlobal("fetch", mock().mockRejectedValue(new Error("boom")));
    await expect(isChromeReachable("http://127.0.0.1:12345", 50)).resolves.toBe(false);
  });

  it("stopMoltBotChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: mock() };
    await stopMoltBotChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopMoltBotChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopMoltBotChrome sends SIGTERM and returns once CDP is down", async () => {
    vi.stubGlobal("fetch", mock().mockRejectedValue(new Error("down")));
    const proc = { killed: false, exitCode: null, kill: mock() };
    await stopMoltBotChrome(
      {
        proc,
        cdpPort: 12345,
