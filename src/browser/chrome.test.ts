...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "razroom-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateRazroomProfile(userDataDir, { color: DEFAULT_RAZROOM_BROWSER_COLOR });
    ...
    expect(def.name).toBe(DEFAULT_RAZROOM_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_RAZROOM_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".razroom-profile-decorated"),
      "utf-8",
    );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateRazroomProfile(userDataDir, { color: "lobster-orange" });
    const localState = await readJson(path.join(userDataDir, "Local State"));
    const profile = localState.profile as Record<string, unknown>;
    const infoCache = profile.info_cache as Record<string, unknown>;
    const def = infoCache.Default as Record<string, unknown>;
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_RAZROOM_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopRazroomChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: mock() };
    await stopRazroomChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopRazroomChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopRazroomChrome sends SIGTERM and returns once CDP is down", async () => {
