...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".moltbot-profile-decorated");
}
 ...
 */
export function decorateMoltBotProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MOLTBOT_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MOLTBOT_BROWSER_COLOR).toUpperCase();
