...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".razroom-profile-decorated");
}
 ...
 */
export function decorateRazroomProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_RAZROOM_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_RAZROOM_BROWSER_COLOR).toUpperCase();
