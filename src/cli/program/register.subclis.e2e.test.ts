...
const { acpAction, registerAcpCli } = vi.hoisted(() => {
  const action = mock();
  const register = mock((program: Command) => {
...
mock("../acp-cli.js", () => ({ registerAcpCli }));
mock("../nodes-cli.js", () => ({ registerNodesCli }));
...
describe("registerSubCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.RAZROOM_DISABLE_LAZY_SUBCOMMANDS;
  ...
  it("re-parses argv for lazy subcommands", async () => {
    process.argv = ["node", "razroom", "nodes", "list"];
    const program = new Command();
    program.name("razroom");
    registerSubCliCommands(program, process.argv);
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    process.argv = ["node", "razroom", "acp", "--help"];
    const program = new Command();
    program.name("razroom");
    registerSubCliCommands(program, process.argv);
    ...
    await program.parseAsync(["node", "razroom", "acp"], { from: "user" });
    expect(registerAcpCli).toHaveBeenCalledTimes(1);
    expect(acpAction).toHaveBeenCalledTimes(1);
  });
});
