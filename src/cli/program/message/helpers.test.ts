...
const messageCommandMock = mock(async () => {});
mock("../../../commands/message.js", () => ({
...
mock("../../plugin-registry.js", () => ({
  ensurePluginRegistryLoaded: mock(),
}));

const hasHooksMock = mock(() => false);
const runGatewayStopMock = mock(async () => {});
const runGlobalGatewayStopSafelyMock = mock(
  async (params: {
    event: { reason?: string };
...
const exitMock = mock((): never => {
  throw new Error("exit");
});
const errorMock = mock();
const runtimeMock = { log: mock(), error: errorMock, exit: exitMock };
mock("../../../runtime.js", () => ({
...
describe("runMessageAction", () => {
  ...
  it("calls exit(0) after successful message delivery", async () => {
    const fakeCommand = { help: mock() } as never;
