...
const getMemorySearchManager = mock();
const loadConfig = mock(() => ({}));
const resolveDefaultAgentId = mock(() => "main");
...
describe("memory cli", () => {
  it("prints vector status when available", async () => {
    ...
    const log = spyOn(defaultRuntime, "log").mockImplementation(() => {});
  ...
  it("prints vector error when unavailable", async () => {
    ...
    getMemorySearchManager.mockResolvedValueOnce({
      manager: {
        probeVectorAvailability: mock(async () => false),
        status: () => ({
          files: 0,
          chunks: 0,
          dirty: true,
  ...
  it("prints embeddings status when deep", async () => {
    ...
    const { defaultRuntime } = await import("../runtime.js");
    const close = mock(async () => {});
    const probeEmbeddingAvailability = mock(async () => ({ ok: true }));
    getMemorySearchManager.mockResolvedValueOnce({
      manager: {
        probeVectorAvailability: mock(async () => true),
  ...
  it("logs close failure after status", async () => {
    ...
    const error = spyOn(defaultRuntime, "error").mockImplementation(() => {});
    const program = new Command();
    program.name("test");
    registerMemoryCli(program);
    await program.parseAsync(["memory", "status"], { from: "user" });
  ...
  it("reindexes on status --index", async () => {
    ...
    spyOn(defaultRuntime, "log").mockImplementation(() => {});
    const program = new Command();
    program.name("test");
    registerMemoryCli(program);
