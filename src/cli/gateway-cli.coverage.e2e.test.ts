...
const callGateway = mock(async () => ({ ok: true }));
const startGatewayServer = mock(async () => ({
  close: mock(async () => {}),
}));
const setVerbose = mock();
const forceFreePortAndWait = mock(async () => ({
  killed: [],
  waitedMs: 0,
  escalatedToSigkill: false,
}));
const serviceIsLoaded = mock().mockResolvedValue(true);
const discoverGatewayBeacons = mock(async () => []);
const gatewayStatusCommand = mock(async () => {});
...
mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: mock(),
    readRuntime: mock().mockResolvedValue({ status: "running" }),
  }),
}));

mock("../daemon/program-args.js", () => ({
  resolveGatewayProgramArguments: async () => ({
    programArguments: ["/bin/node", "cli", "gateway", "--port", "18789"],
  }),
}));

mock("../infra/bonjour-discovery.js", () => ({
  discoverGatewayBeacons: (opts: unknown) => discoverGatewayBeacons(opts),
}));

mock("../commands/gateway-status.js", () => ({
...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints human output with details on new lines", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (Razroom)",
        displayName: "Studio",
        domain: "razroom.internal.",
        host: "studio.razroom.internal",
        lanHost: "studio.local",
        tailnetDns: "studio.tailnet.ts.net",
        gatewayPort: 18789,
        sshPort: 22,
    ...
    expect(out).toContain("- Studio razroom.internal.");
    expect(out).toContain("  tailnet: studio.tailnet.ts.net");
    expect(out).toContain("  host: studio.razroom.internal");
    expect(out).toContain("  ws: ws://studio.razroom.internal:18789");
  });
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ RAZROOM_GATEWAY_PORT: "19001" }, async () => {
      runtimeLogs.length = 0;
      runtimeErrors.length = 0;
