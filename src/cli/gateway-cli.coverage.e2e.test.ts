...
describe("gateway-cli coverage", () => {
  ...
  it("registers gateway discover and prints human output with details on new lines", async () => {
    ...
    expect(out).toContain("  host: studio.moltbot.internal");
    expect(out).toContain("  ws: ws://studio.moltbot.internal:18789");
  });
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ MOLTBOT_GATEWAY_PORT: "19001" }, async () => {
      runtimeLogs.length = 0;
      runtimeErrors.length = 0;
