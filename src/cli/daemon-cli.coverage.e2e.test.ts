...
const resolveGatewayProgramArguments = mock(async () => ({
  programArguments: ["/bin/node", "cli", "gateway", "--port", "18789"],
}));
const serviceInstall = mock().mockResolvedValue(undefined);
const serviceUninstall = mock().mockResolvedValue(undefined);
const serviceStop = mock().mockResolvedValue(undefined);
const serviceRestart = mock().mockResolvedValue(undefined);
const serviceIsLoaded = mock().mockResolvedValue(false);
const serviceReadCommand = mock().mockResolvedValue(null);
const serviceReadRuntime = mock().mockResolvedValue({ status: "running" });
const findExtraGatewayServices = mock(async () => []);
const inspectPortUsage = mock(async (port: number) => ({
  port,
  status: "free",
  listeners: [],
  hints: [],
...
mock("../daemon/legacy.js", () => ({
  findLegacyGatewayServices: async () => [],
}));

mock("../daemon/inspect.js", () => ({
