...
const listChannelPairingRequests = mock();
const approveChannelPairingCode = mock();
const notifyPairingApproved = mock();
const pairingIdLabels: Record<string, string> = {
  telegram: "telegramUserId",
  discord: "discordUserId",
};
const normalizeChannelId = mock((raw: string) => {
  if (!raw) {
    return null;
  }
  if (raw === "imsg") {
...
const getPairingAdapter = mock((channel: string) => ({
  idLabel: pairingIdLabels[channel] ?? "userId",
}));
const listPairingChannels = mock(() => ["telegram", "discord", "imessage"]);
...
mock("../channels/plugins/pairing.js", () => ({
  listPairingChannels,
