...
const acquireGatewayLock = mock(async () => ({
  release: mock(async () => {}),
}));
const consumeGatewaySigusr1RestartAuthorization = mock(() => true);
const isGatewaySigusr1RestartExternallyAllowed = mock(() => false);
const markGatewaySigusr1RestartHandled = mock();
const getActiveTaskCount = mock(() => 0);
const waitForActiveTasks = mock(async () => ({ drained: true }));
const resetAllLanes = mock();
...
mock("../../infra/gateway-lock.js", () => ({
  acquireGatewayLock: () => acquireGatewayLock(),
}));

mock("../../infra/restart.js", () => ({
