...
mock("./web-fetch-utils.js", async () => {
  const actual =
    await vi.importActual<typeof import("./web-fetch-utils.js")>("./web-fetch-utils.js");
  return {
    ...actual,
    extractReadableContent: mock().mockResolvedValue({
      title: "HTML Page",
      text: "HTML Page\n\nContent here.",
    }),
  };
...
describe("web_fetch Cloudflare Markdown for Agents", () => {
  ...
  beforeEach(() => {
    ...
    spyOn(ssrf, "resolvePinnedHostname").mockImplementation((hostname) =>
  ...
  it("sends Accept header preferring text/markdown", async () => {
    const fetchSpy = mock().mockResolvedValue(markdownResponse("# Test Page\n\nHello world."));
  ...
  it("uses cf-markdown extractor for text/markdown responses", async () => {
    ...
    const fetchSpy = mock().mockResolvedValue(markdownResponse(md));
    // @ts-expect-error mock fetch
    global.fetch = fetchSpy;
  ...
  it("falls back to readability for text/html responses", async () => {
    ...
    const fetchSpy = mock().mockResolvedValue(htmlResponse(html));
    // @ts-expect-error mock fetch
    global.fetch = fetchSpy;
  ...
  it("logs x-markdown-tokens when header is present", async () => {
    const logSpy = spyOn(logger, "logDebug").mockImplementation(() => {});
    const fetchSpy = vi
      .fn()
  ...
  it("does not log x-markdown-tokens when header is absent", async () => {
    const logSpy = spyOn(logger, "logDebug").mockImplementation(() => {});
    const fetchSpy = mock().mockResolvedValue(markdownResponse("# No tokens"));
    // @ts-expect-error mock fetch
    global.fetch = fetchSpy;
