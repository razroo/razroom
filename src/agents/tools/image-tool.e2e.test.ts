...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(process.cwd(), ".razroom-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): RazroomConfig {
  return {
    agents: {
      defaults: {
...
describe("image tool implicit imageModel config", () => {
  ...
  it("stays disabled without auth when no pairing is possible", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-image-"));
  ...
  it("allows workspace images via createRazroomCodingTools default workspace root", async () => {
    await withTempWorkspacePng(async ({ imagePath }) => {
      ...
      try {
        ...
        const tools = createRazroomCodingTools({ config: cfg, agentDir });
  ...
  it("sandboxes image paths like the read tool", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-image-sandbox-"));
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    ...
    const res = await tool.execute("t1", {
      prompt: "Describe the image.",
      image: "@/Users/steipete/.razroom/media/inbound/photo.png",
    });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  it("calls /v1/coding_plan/vlm for minimax image models", async () => {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg: RazroomConfig = {
      agents: { defaults: { model: { primary: "minimax/MiniMax-M2.1" } } },
    };
