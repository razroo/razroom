...
const browserClientMocks = vi.hoisted(() => ({
  browserCloseTab: mock(async () => ({})),
  browserFocusTab: mock(async () => ({})),
  browserOpenTab: mock(async () => ({})),
  browserProfiles: mock(async () => []),
  browserSnapshot: mock(async () => ({
    ok: true,
    format: "ai",
    targetId: "t1",
  ...
  browserStart: mock(async () => ({})),
  browserStatus: mock(async () => ({
    ok: true,
    running: true,
    pid: 1,
    cdpPort: 18792,
  ...
  browserStop: mock(async () => ({})),
  browserTabs: mock(async () => []),
}));
mock("../../browser/client.js", () => browserClientMocks);

const browserActionsMocks = vi.hoisted(() => ({
  browserAct: mock(async () => ({ ok: true })),
  browserArmDialog: mock(async () => ({ ok: true })),
  browserArmFileChooser: mock(async () => ({ ok: true })),
  browserConsoleMessages: mock(async () => ({
    ok: true,
    targetId: "t1",
    messages: [
      {
  ...
  browserNavigate: mock(async () => ({ ok: true })),
  browserPdfSave: mock(async () => ({ ok: true, path: "/tmp/test.pdf" })),
  browserScreenshotAction: mock(async () => ({ ok: true, path: "/tmp/test.png" })),
}));
mock("../../browser/client-actions.js", () => browserActionsMocks);

const browserConfigMocks = vi.hoisted(() => ({
  resolveBrowserConfig: mock(() => ({
    enabled: true,
    controlPort: 18791,
  })),
}));
mock("../../browser/config.js", () => browserConfigMocks);

const nodesUtilsMocks = vi.hoisted(() => ({
  listNodes: mock(async () => []),
}));
mock("./nodes-utils.js", async () => {
...
const gatewayMocks = vi.hoisted(() => ({
  callGatewayTool: mock(async () => ({
    ok: true,
    payload: { result: { ok: true, running: true } },
  })),
}));
mock("./gateway.js", () => gatewayMocks);
...
const toolCommonMocks = vi.hoisted(() => ({
  imageResultFromFile: mock(),
}));
mock("./common.js", async () => {
