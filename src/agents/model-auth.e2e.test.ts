...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-oauth-"));

    try {
      process.env.RAZROOM_STATE_DIR = tempDir;
      process.env.RAZROOM_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.RAZROOM_AGENT_DIR, {
