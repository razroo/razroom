...
mock("./pi-embedded-helpers.js", async () => {
  const actual = await vi.importActual("./pi-embedded-helpers.js");
  return {
    ...actual,
    isGoogleModelApi: mock(),
    sanitizeSessionMessagesImages: mock().mockImplementation(async (msgs) => msgs),
  };
});

describe("sanitizeSessionHistory e2e smoke", () => {
  const mockSessionManager = {
    getEntries: mock().mockReturnValue([]),
