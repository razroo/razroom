...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.RAZROOM_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".razroom", `workspace-${profile}`);
  }
  return path.join(home, ".razroom", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".razroom";
