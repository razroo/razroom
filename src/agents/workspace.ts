...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.MOLTBOT_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".moltbot", `workspace-${profile}`);
  }
  return path.join(home, ".moltbot", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".moltbot";
