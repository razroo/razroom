...
describe("runWithModelFallback", () => {
  it("normalizes openai gpt-5.3 codex to openai-codex before running", async () => {
    const cfg = makeCfg();
    const run = mock().mockResolvedValueOnce("ok");
  ...
  it("does not fall back on non-auth errors", async () => {
    const cfg = makeCfg();
    const run = mock().mockRejectedValueOnce(new Error("bad request")).mockResolvedValueOnce("ok");
  ...
  it("skips providers when all profiles are in cooldown", async () => {
    ...
    const run = mock().mockImplementation(async (providerId, modelId) => {
  ...
  it("does not skip when any profile is available", async () => {
    ...
    const run = mock().mockImplementation(async (providerId) => {
