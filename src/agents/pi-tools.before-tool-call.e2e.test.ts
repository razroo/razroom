...
describe("before_tool_call hook integration", () => {
  let hookRunner: {
    hasHooks: ReturnType<typeof mock>;
    runBeforeToolCall: ReturnType<typeof mock>;
  };
  ...
  it("executes tool normally when no hook is registered", async () => {
    ...
    const execute = mock().mockResolvedValue({ content: [], details: { ok: true } });
    // oxlint-disable-next-line typescript/no-explicit-any
...
describe("before_tool_call hook deduplication (#15502)", () => {
  ...
  beforeEach(() => {
    hookRunner = {
      hasHooks: mock(() => true),
      runBeforeToolCall: mock(async () => undefined),
    };
    // oxlint-disable-next-line typescript/no-explicit-any
    mockGetGlobalHookRunner.mockReturnValue(hookRunner as any);
  });
