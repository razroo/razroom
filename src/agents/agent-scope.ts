...
export function listAgentIds(cfg: RazroomConfig): string[] {
...
export function resolveDefaultAgentId(cfg: RazroomConfig): string {
  const agents = listAgents(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
export function resolveSessionAgentIds(params: { sessionKey?: string; config?: RazroomConfig }): {
...
export function resolveAgentModelPrimary(cfg: RazroomConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: RazroomConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: RazroomConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
