...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses RAZROOM_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      RAZROOM_HOME: "/srv/razroom-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/razroom-home"), ".razroom", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".razroom", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("razroom-workspace-");
