...
function resolveMainSessionKeyForSandbox(params: { cfg?: RazroomConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: RazroomConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  lines.push(
    `- See: ${formatCliCommand(`razroom sandbox explain --session ${runtime.sessionKey}`)}`,
  );
