...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "razroom.sandbox=1");
  args.push("--label", `razroom.sessionKey=${params.scopeKey}`);
  args.push("--label", `razroom.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `razroom.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "razroom.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`razroom sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`razroom sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("razroom sandbox recreate --all");
}
