...
mock("../gateway/call.js", () => ({ callGateway }));
mock("../media/image-ops.js", () => ({
  getImageMetadata: mock(async () => ({ width: 1, height: 1 })),
  resizeToJpeg: mock(async () => Buffer.from("jpeg")),
}));
...
describe("nodes camera_snap", () => {
  ...
  it("maps jpg payloads to image/jpeg", async () => {
    ...
    const tool = createRazroomTools().find((candidate) => candidate.name === "nodes");
    if (!tool) {
