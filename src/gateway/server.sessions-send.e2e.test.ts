...
import { createRazroomTools } from "../agents/razroom-tools.js";
...
beforeAll(async () => {
  ...
  process.env.RAZROOM_GATEWAY_PORT = String(gatewayPort);
  process.env.RAZROOM_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createRazroomTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
