...
mock("../session-utils.js", async () => {
  const actual = await vi.importActual<typeof import("../session-utils.js")>("../session-utils.js");
  return {
    ...actual,
    loadCombinedSessionStoreForGateway: mock(() => ({ storePath: "(multiple)", store: {} })),
  };
});

mock("../../infra/session-cost-usage.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/session-cost-usage.js")>(
    "../../infra/session-cost-usage.js",
  );
  return {
    ...actual,
    discoverAllSessions: mock(async (params?: { agentId?: string }) => {
    ...
    loadSessionCostSummary: mock(async () => ({
      input: 0,
      output: 0,
      cacheRead: 0,
      cacheWrite: 0,
    ...
    loadSessionUsageTimeSeries: mock(async () => ({
      sessionId: "s-opus",
      points: [],
    })),
    loadSessionLogs: mock(async () => []),
  };
});
...
describe("sessions.usage", () => {
  ...
  it("resolves store entries by sessionId when queried via discovered agent-prefixed key", async () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "razroom-usage-test-"));
