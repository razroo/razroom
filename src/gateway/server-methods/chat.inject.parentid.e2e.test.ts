...
describe("gateway chat.inject transcript writes", () => {
  it("appends a Pi session entry that includes parentId", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "razroom-chat-inject-"));
    ...
    const context: InjectCtx = {
      broadcast: mock() as unknown as InjectCtx["broadcast"],
      nodeSendToSession: mock() as unknown as InjectCtx["nodeSendToSession"],
    };
