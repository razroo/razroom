...
describe("agent event handler", () => {
  function createHarness(params?: {
    ...
    const nowSpy =
      params?.now === undefined ? undefined : spyOn(Date, "now").mockReturnValue(params.now);
    const broadcast = mock();
    const broadcastToConnIds = mock();
    const nodeSendToSession = mock();
