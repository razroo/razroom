...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      ...
      const prevHandshakeTimeout = process.env.RAZROOM_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.RAZROOM_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.RAZROOM_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.RAZROOM_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const identity = loadOrCreateDeviceIdentity(
        path.join(os.tmpdir(), `razroom-test-device-${randomUUID()}.json`),
      );
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.RAZROOM_GATEWAY_TOKEN;
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "razroom-device-scope-"));
