...
const sessionCleanupMocks = vi.hoisted(() => ({
  clearSessionQueues: mock(() => ({ followupCleared: 0, laneCleared: 0, keys: [] })),
  stopSubagentsForRequester: mock(() => ({ stopped: 0 })),
}));

const sessionHookMocks = vi.hoisted(() => ({
  triggerInternalHook: mock(async () => {}),
}));

mock("../auto-reply/reply/queue.js", async () => {
...
mock("../auto-reply/reply/abort.js", async () => {
...
mock("../hooks/internal-hooks.js", async () => {
...
describe("gateway server sessions", () => {
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-sessions-"));
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("sessions.preview resolves legacy mixed-case main alias with custom mainKey", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-sessions-preview-alias-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
  ...
  test("sessions.resolve and mutators clean legacy main-alias ghost keys", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "razroom-sessions-cleanup-alias-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
    testState.agentsConfig = { list: [{ id: "ops", default: true }] };
    testState.sessionConfig = { mainKey: "work" };
