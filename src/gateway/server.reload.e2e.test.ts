...
const hoisted = vi.hoisted(() => {
  ...
  const browserStop = mock(async () => {});
  const startBrowserControlServerIfEnabled = mock(async () => ({
    stop: browserStop,
  }));

  const heartbeatStop = mock();
  const heartbeatUpdateConfig = mock();
  const startHeartbeatRunner = mock(() => ({
    stop: heartbeatStop,
    updateConfig: heartbeatUpdateConfig,
  }));

  const startGmailWatcher = mock(async () => ({ started: true }));
  const stopGmailWatcher = mock(async () => {});

  const providerManager = {
    getRuntimeSnapshot: mock(() => ({
      providers: {
        whatsapp: {
          running: false,
          connected: false,
    ...
    startChannels: mock(async () => {}),
    startChannel: mock(async () => {}),
    stopChannel: mock(async () => {}),
    markChannelLoggedOut: mock(),
  };

  const createChannelManager = mock(() => providerManager);

  const reloaderStop = mock(async () => {});
  ...
  const startGatewayConfigReloader = mock(
