...
describe("POST /tools/invoke", () => {
  ...
  it("routes tools invoke before plugin HTTP handlers", async () => {
    const pluginHandler = mock(async (_req: IncomingMessage, res: ServerResponse) => {
      res.statusCode = 418;
      res.end("plugin");
      return true;
    });
