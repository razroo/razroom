import { afterAll, afterEach, beforeAll, describe, expect, test, mock, spyOn } from "bun:test";
...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  beforeAll(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "razroom-session-fs-test-"));
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __razroom?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__razroom?.kind).toBe("compaction");
    expect(marker.__razroom?.id).toBe("comp-1");
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  beforeAll(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "razroom-session-preview-test-"));
    storePath = path.join(tmpDir, "sessions.json");
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses RAZROOM_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/razroom-home"), ".razroom", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test("keeps cross-agent absolute sessionFile when storePath agent context differs", () => {
    const storePath = "/tmp/razroom/agents/main/sessions/sessions.json";
    const sessionFile = "/tmp/razroom/agents/ops/sessions/sess-safe.jsonl";
  ...
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/razroom/agents/main/sessions/sessions.json",
    );
...
describe("archiveSessionTranscripts", () => {
  ...
  beforeAll(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "razroom-archive-test-"));
    storePath = path.join(tmpDir, "sessions.json");
    vi.stubEnv("RAZROOM_HOME", tmpDir);
  });
