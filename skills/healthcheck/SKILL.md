---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Razroom deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Razroom cron scheduling for periodic checks, or version status checks on a machine running Razroom (laptop, workstation, Pi, VPS).
---

# Razroom Host Hardening

## Overview

Assess and harden the host running Razroom, then align it to a user-defined risk tolerance without breaking access. Use Razroom security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Razroom changes the host firewall, SSH, or OS updates; it does not.
...
5. Razroom gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Razroom can access sensitive data.
...
### 2) Run Razroom security audits (read-only)

As part of the default read-only checks, run `razroom security audit --deep`. Only offer alternatives if the user requests them:

1. `razroom security audit` (faster, non-probing)
2. `razroom security audit --json` (structured output)

Offer to apply Razroom safe defaults (numbered):

1. `razroom security audit --fix`

Be explicit that `--fix` only tightens Razroom defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Razroom version/update status (read-only)

As part of the default read-only checks, run `razroom update status`.
...
- Credential hygiene notes (location of Razroom creds, prefer disk encryption)
...
After Razroom install or first hardening pass, run at least one baseline audit and version check:

- `razroom security audit`
- `razroom security audit --deep`
- `razroom update status`

Ongoing monitoring is recommended. Use the Razroom cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `razroom cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `razroom update status`
...
Before creating, `razroom cron list` and match on exact `name`. If found, `razroom cron edit <id> ...`.
If not found, `razroom cron add --name <name> ...`.
...
- `razroom security audit [--deep] [--fix] [--json]`
- `razroom status` / `razroom status --deep`
- `razroom health --json`
- `razroom update status`
- `razroom cron add|list|runs|run`

Do not invent CLI flags or imply Razroom enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Razroom compaction:
