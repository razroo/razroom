---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for MoltBot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, MoltBot cron scheduling for periodic checks, or version status checks on a machine running MoltBot (laptop, workstation, Pi, VPS).
---

# MoltBot Host Hardening

## Overview

Assess and harden the host running MoltBot, then align it to a user-defined risk tolerance without breaking access. Use MoltBot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim MoltBot changes the host firewall, SSH, or OS updates; it does not.
...
5. MoltBot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if MoltBot can access sensitive data.
...
### 2) Run MoltBot security audits (read-only)

As part of the default read-only checks, run `moltbot security audit --deep`. Only offer alternatives if the user requests them:

1. `moltbot security audit` (faster, non-probing)
2. `moltbot security audit --json` (structured output)

Offer to apply MoltBot safe defaults (numbered):

1. `moltbot security audit --fix`

Be explicit that `--fix` only tightens MoltBot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check MoltBot version/update status (read-only)

As part of the default read-only checks, run `moltbot update status`.
...
- Credential hygiene notes (location of MoltBot creds, prefer disk encryption)
...
After MoltBot install or first hardening pass, run at least one baseline audit and version check:

- `moltbot security audit`
- `moltbot security audit --deep`
- `moltbot update status`

Ongoing monitoring is recommended. Use the MoltBot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `moltbot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `moltbot update status`
...
Before creating, `moltbot cron list` and match on exact `name`. If found, `moltbot cron edit <id> ...`.
If not found, `moltbot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `moltbot update status` (preferred for source checkouts and channels)
2. `npm view moltbot version` (published npm version)

## MoltBot command accuracy

Use only supported commands and flags:

- `moltbot security audit [--deep] [--fix] [--json]`
- `moltbot status` / `moltbot status --deep`
- `moltbot health --json`
- `moltbot update status`
- `moltbot cron add|list|runs|run`

Do not invent CLI flags or imply MoltBot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by MoltBot compaction:
